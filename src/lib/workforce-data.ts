...
const WORKFORCE_MAX_PRIOR_OUTPUTS = 2;

const WORKFORCE_MAX_PRIOR_OUTPUT_CHARS = 900;

const WORKFORCE_MAX_EVIDENCE_ITEMS = 2;

const WORKFORCE_MAX_EVIDENCE_CHARS = 1_200;

const WORKFORCE_MAX_HANDOFF_CONTEXT_TEXT_CHARS = 1_200;
...
export const LEGACY_EMPLOYEE_KEY_ALIASES = {
  lead_intake_coordinator: "lead-intake-coordinator",

  product_intelligence_analyst: "product-intelligence-analyst",
} as const;

export function canonicalEmployeeKey(value: string): string {
  const key = value.trim();

  return LEGACY_EMPLOYEE_KEY_ALIASES[key as keyof typeof LEGACY_EMPLOYEE_KEY_ALIASES] ?? key;
}

function legacyKeysForCanonicalEmployee(canonicalKey: string): string[] {
  return Object.entries(LEGACY_EMPLOYEE_KEY_ALIASES)
    .filter(([, mappedCanonicalKey]) => mappedCanonicalKey === canonicalKey)
...
export type WorkforceExecutionKind = "language_model" | "tool" | "deterministic";
...
export const COSSA_GROWTH_WORKFORCE = [
  {
    ...
    system_instructions: `${INTERNAL_WORK_RULES.join(" ")} Use official Cossa web properties and authorised website data only. A public website check proves only what was actually observed. Safe analysis and recommendations should continue automatically. Hand technical implementation requirements to the Website Delivery Specialist or Tech Solutions Specialist.`,
...

  {
    ...
    system_instructions: `${INTERNAL_WORK_RULES.join(" ")} Build practical social and digital growth plans. Routine planning does not require owner approval. Every useful strategy should hand clear requirements to the Content Writer, Creative Media Producer and Social Media Manager.`,
  ...
  {
    employee_key: "creative-media-producer",

    name: "Creative Media Producer",

    title: "AI Creative Media Producer",

    department: "Growth & Creative",
...

  {
    ...
    system_instructions: `${INTERNAL_WORK_RULES.join(" ")} Routine internal scheduling does not require owner approval. Ensure each social package contains copy, platform, timing and the required visual asset or visual-production requirement before handing it to the Social Media Manager.`,
  ...
  {
    employee_key: "social-media-manager",

    name: "Social Media Manager",

    title: "AI Social Media Manager",

    department: "Growth & Social Media",
...

  {
    ...
    prohibited_actions: [...HIGH_RISK_ACTIONS, "fabricate performance metrics", "buy engagement"],

    system_instructions: `${INTERNAL_WORK_RULES.join(" ")} Analyse only connected and authorised data. Missing data should create a clear integration requirement instead of guessed metrics. Routine internal analysis should continue without owner interruption.`,
...

  {
    ...
    kpis: ["Clear campaign logic.", "No fabricated advertising metrics.", "No unapproved spend."],
    ...
    system_instructions: `${INTERNAL_WORK_RULES.join(" ")} Strategy, research, ad creation and optimisation recommendations may continue internally. Actual spend, campaign launch, bid changes and budget changes require owner approval.`,
...

  {
    ...
    title: "AI Store Operations Manager",

    department: "Cossa Store",
    ...
    system_instructions: `${INTERNAL_WORK_RULES.join(" ")} Operate as the Cossa Store internal workflow manager. Keep catalogue, product, supplier, creative and social-commerce work moving hand-to-hand. Safe internal store work does not require owner approval. Supplier orders, payments and binding commercial commitments do.`,
...

  {
    ...
    system_instructions: `${INTERNAL_WORK_RULES.join(" ")} Use legitimate catalogue and market evidence. Product intelligence is internal decision support. Do not describe a trend, demand level, stock position or supplier as verified unless evidence supports it.`,
...

  {
    ...
    title: "AI Supplier Sourcing Analyst",
    ...
    system_instructions: `${INTERNAL_WORK_RULES.join(" ")} Real supplier discovery requires authorised research sources. Record source evidence, operating location, product relevance, contact source and verification date. Do not order, pay, negotiate binding terms or claim a supplier is verified without evidence.`,
  ...
  {
    employee_key: "tech-solutions-specialist",

    name: "Tech Solutions Specialist",

    title: "AI Tech Solutions Specialist",

    department: "Cossa Tech",
    ...
    system_instructions: `${INTERNAL_WORK_RULES.join(" ")} Operate as Cossa Tech's technical coordination specialist. Safe technical analysis, planning, drafting and implementation preparation should continue internally. Route website implementation to Website Delivery and content or creative requirements to the relevant workers.`,
  ...
  {
    employee_key: "website-delivery-specialist",

    name: "Website Delivery Specialist",

    title: "AI Website Delivery Specialist",
    ...
    system_instructions: `${INTERNAL_WORK_RULES.join(" ")} If Cossa or a client needs a website, own the internal website-delivery workflow. Coordinate technical implementation, content, visuals and SEO. Do not leave the request waiting merely because several workers are required. Production domain, DNS, credentials and irreversible changes remain approval-controlled.`,
  ...
  {
    employee_key: "lead-hunter",

    name: "Lead Hunter",

    title: "AI Revenue Lead Hunter & Opportunity Intelligence Specialist",

    department: "Revenue Acquisition",
  ...
  {
    employee_key: "lead-intake-coordinator",

    name: "Lead Intake Coordinator",

    title: "AI Lead Intake Coordinator",
  ...
  {
    employee_key: "sales-conversion-specialist",

    name: "Sales & Conversion Specialist",

    title: "AI Sales & Conversion Specialist",
...

  {
    ...
    system_instructions: `${INTERNAL_WORK_RULES.join(" ")} Analyse authorised customer records and respect consent and opt-outs. Internal reactivation analysis should proceed automatically. Hand valid opportunities to Lead Intake and Sales & Conversion. Actual external communication must use an authorised communication workflow.`,
...

  {
    ...
    system_instructions: `${INTERNAL_WORK_RULES.join(" ")} Produce evidence-backed commercial intelligence. Safe internal opportunity research and matching should proceed automatically. Customer-type opportunities should move through Lead Intake and Sales & Conversion. External introductions, negotiations and commitments require an authorised workflow and appropriate approval.`,
...

  {
    ...
    system_instructions: `${INTERNAL_WORK_RULES.join(" ")} Internal tender and procurement screening should continue automatically when evidence exists. Preserve official source, reference, service-match and closing-date evidence. Tender submission, signed commitments, declarations and binding pricing require owner approval.`,
...
function assertWorkforceProfileIntegrity(): void {
  ...
  for (const profile of COSSA_GROWTH_WORKFORCE) {
    const canonicalKey = canonicalEmployeeKey(profile.employee_key);

    if (canonicalKey !== profile.employee_key) {
      throw new Error(
    ...
    if (seen.has(canonicalKey)) {
      throw new Error(`Duplicate source workforce employee key detected: "${canonicalKey}".`);
    }
...
const GROWTH_WORKFLOW_DEFINITION: WorkforceMissionDefinition = {
  prefix: "Growth coordination:",
  ...
  stages: [
...

    {
      ...
      reason: "Organise complete copy-and-creative packages into a practical channel schedule.",
    },
...
const STORE_WORKFLOW_DEFINITION: WorkforceMissionDefinition = {
  ...
  stages: [
...

    {
      ...
      reason: "Analyse authorised store and campaign evidence and recommend growth improvements.",
    },
...
const TECH_WORKFLOW_DEFINITION: WorkforceMissionDefinition = {
  prefix: "Cossa Tech delivery:",
...
const REVENUE_WORKFLOW_DEFINITION: WorkforceMissionDefinition = {
  prefix: "Revenue acquisition:",
...
const REACTIVATION_WORKFLOW_DEFINITION: WorkforceMissionDefinition = {
  prefix: "Customer reactivation:",
...
const BROKER_DEAL_WORKFLOW_DEFINITION: WorkforceMissionDefinition = {
  prefix: "Commercial deal intelligence:",
  ...
  stages: [
...

    {
      ...
      reason: "Review strategic fit, commercial risks and owner-controlled decisions.",
    },
  ],
...
const PROCUREMENT_WORKFLOW_DEFINITION: WorkforceMissionDefinition = {
  prefix: "Procurement intelligence:",
...
export type CreateGrowthCoordinationMissionInput = CreateCoordinationMissionInput;

export type GrowthCoordinationMissionResult = CoordinationMissionResult;
...
function compactContextRecord(
  ...
  try {
    ...
    if (serialised.length <= WORKFORCE_MAX_HANDOFF_CONTEXT_TEXT_CHARS) {
      return context;
    }
    ...
    return {
      ...
      summary: serialised.slice(0, WORKFORCE_MAX_HANDOFF_CONTEXT_TEXT_CHARS),
    };
  } catch {
    return {
      compacted: true,

      summary: "Caller context could not be serialised.",
    };
  }
}

function stageNumberFromContext(context: Record<string, unknown>): number | null {
  const raw = context.stage;
...
function executionOrderFromContext(context: Record<string, unknown>): string | null {
  return typeof context.execution_order === "string" ? context.execution_order : null;
}
...
): Promise<Pick<EmployeeHandoff, "id" | "status" | "created_at">[]> {
  return rows<Pick<EmployeeHandoff, "id" | "status" | "created_at">>(
...
async function countPendingMissionApprovals(
  ...
  const pending = await rows<Pick<Approval, "id">>(
...
async function assertPriorStagesCompleted({
  ...
  const missionHandoffs = await rows<
    Pick<EmployeeHandoff, "id" | "status" | "context" | "created_at">
  >(
  ...
  const target = missionHandoffs.find((handoff) => handoff.id === handoffId);

  if (!target) {
    throw new Error("The workforce handoff was not found while validating stage order.");
  }

  if (executionOrderFromContext(target.context) !== "strict_sequential") {
    return;
  }

  const targetStage = stageNumberFromContext(target.context);
  ...
  for (const handoff of missionHandoffs) {
    if (handoff.id === target.id) {
      continue;
    }

    const stage = stageNumberFromContext(handoff.context);
...
export async function getEmployeeById(
  ...
  const validEmployeeId = requireNonEmptyValue(employeeId, "Employee ID");
  ...
  if (error) {
    throw createDatabaseError("Unable to load AI employee", error);
  }

  if (!data) {
    throw new Error("AI employee was not found.");
  }
...
export async function getEmployeeByKey(
  ...
  const requestedKey = requireNonEmptyValue(employeeKey, "Employee key");

  const canonicalKey = canonicalEmployeeKey(requestedKey);

  const {
    data: canonicalEmployee,
    ...
    .select("*")
    .eq("organisation_id", organisationId)
    .eq("employee_key", canonicalKey)
    .maybeSingle();

  if (canonicalError) {
    throw createDatabaseError("Unable to load AI employee", canonicalError);
  }

  if (canonicalEmployee) {
    return canonicalEmployee as AiEmployee;
  }

  const legacyKeys = legacyKeysForCanonicalEmployee(canonicalKey);

  if (legacyKeys.length > 0) {
    const legacyEmployees = await rows<AiEmployee>(
  ...
  throw new Error(`AI employee "${canonicalKey}" was not found.`);
}
...
async function resolveAssignmentEmployee(
  ...
  if (input.employeeId?.trim()) {
    return getEmployeeById(input.employeeId, organisationId);
  }

  if (input.employeeKey?.trim()) {
    return getEmployeeByKey(input.employeeKey, organisationId);
  }

  throw new Error("Employee ID or employee key is required.");
}
...
export function listEmployeeAssignedMissions(
  ...
  return rows<Mission>(
    ...
    db
      ...
      .select("*")
      .eq("organisation_id", organisationId)
      .eq("assigned_employee_id", validEmployeeId)
      .order("created_at", {
        ascending: false,
      }),
  );
...
function employeeMapByCanonicalKey(employees: AiEmployee[]): Map<string, AiEmployee> {
  const result = new Map<string, AiEmployee>();
  ...
  for (const employee of employees) {
    ...
    if (!existing) {
      result.set(canonicalKey, employee);

      continue;
    }

    const employeeIsCanonical = employee.employee_key === canonicalKey;

    const existingIsCanonical = existing.employee_key === canonicalKey;

    if (employeeIsCanonical && !existingIsCanonical) {
      result.set(canonicalKey, employee);
    }
  }
...
async function migrateUnambiguousLegacyEmployeeKeys(
  ...
  const exactKeys = new Set(existing.map((employee) => employee.employee_key));

  for (const [legacyKey, canonicalKey] of Object.entries(LEGACY_EMPLOYEE_KEY_ALIASES)) {
    const legacyEmployee = existing.find((employee) => employee.employee_key === legacyKey);

    if (!legacyEmployee) {
      continue;
    }

    if (exactKeys.has(canonicalKey)) {
      console.warn(
    ...
    const { error } = await db
      .from("ai_employees")
      .update({
        employee_key: canonicalKey,

        updated_at: new Date().toISOString(),
      })
      .eq("organisation_id", organisationId)
      .eq("id", legacyEmployee.id)
      .eq("employee_key", legacyKey);

    if (error) {
      throw createDatabaseError(`Unable to migrate legacy employee key "${legacyKey}"`, error);
    }

    exactKeys.delete(legacyKey);

    exactKeys.add(canonicalKey);
  }
}
...
async function synchroniseKnownProfiles(
  ...
  const existingByCanonicalKey = employeeMapByCanonicalKey(existing);

  for (const profile of COSSA_GROWTH_WORKFORCE) {
    const existingEmployee = existingByCanonicalKey.get(profile.employee_key);
    ...
    const nextStatus: EmployeeStatus =
      existingEmployee.status === "paused" || existingEmployee.status === "retired"
    ...
    const { error } = await db
      ...
      .update({
        ...
        requires_approval_by_default: profile.requires_approval_by_default,
    ...
    if (error) {
      throw createDatabaseError(`Unable to synchronise ${profile.name}`, error);
    }
  }
}
...
export function listMissions(organisationId = COSSA_ORGANISATION_ID): Promise<Mission[]> {
  return rows<Mission>(
    ...
    db.from("missions").select("*").eq("organisation_id", organisationId).order("created_at", {
      ascending: false,
    }),
  );
}
...
export function listWorkforceRuns(organisationId = COSSA_ORGANISATION_ID): Promise<MissionRun[]> {
  return rows<MissionRun>(
    ...
    db.from("mission_runs").select("*").eq("organisation_id", organisationId).order("created_at", {
      ascending: false,
    }),
  );
}
...
export async function queueMission(
  ...
  if (!data) {
    throw new Error("Unable to queue mission: Mission was not found or is not in draft status");
  }
...
export async function createDirectEmployeeMission(
  input: CreateDirectEmployeeMissionInput,

  organisationId = COSSA_ORGANISATION_ID,
): Promise<DirectEmployeeMissionResult> {
  const objective = requireNonEmptyValue(input.objective, "Task objective");

  const employee = await resolveAssignmentEmployee(input, organisationId);

  if (employee.status !== "active") {
    throw new Error(`${employee.name} is ${employee.status} and cannot receive a new task.`);
  }
  ...
  const title =
    input.title?.trim() || `Employee assignment: ${employee.name} — ${objective.slice(0, 90)}`;
  ...
  const mission = await createMission(
    {
      ...
      business_unit_id: input.business_unit_id ?? employee.business_unit_id ?? null,
      ...
      constraints: [
        ...
        ...(canonicalEmployeeKey(employee.employee_key) === "lead-hunter"
          ? [
      ...
      output_schema: {
        assignment_mode: "direct_employee",

        assigned_employee_key: canonicalEmployeeKey(employee.employee_key),

        assigned_employee_name: employee.name,

        collaboration_mode: "single_employee",

        safe_internal_work: "continue_automatically",

        external_actions_enabled: false,

        owner_interruption: "high_risk_only",
  ...
  const {
    ...
    .insert({
      ...
      context: {
        ...
        employee_key: canonicalEmployeeKey(employee.employee_key),

        previous_employee_key: null,

        next_employee_key: null,

        workflow: "Direct employee assignment",

        collaboration_mode: "single_employee",

        execution_order: "single_stage",

        safe_internal_work: "continue",

        owner_interruption: "high_risk_only",

        external_actions_enabled: false,

        caller_context: compactContextRecord(input.context),
      },
  ...
  if (handoffError) {
    const { error: missionFailureError } = await db
      .from("missions")
      .update({
        status: "failed",
    ...
    if (missionFailureError) {
      console.error("Unable to mark direct assignment mission as failed", missionFailureError);
    }

    throw createDatabaseError("Unable to create the direct employee handoff", handoffError);
  }

  if (!handoffData) {
    throw new Error("The employee mission was created but no handoff record was returned.");
  }

  let queuedMission: Mission;

  try {
    queuedMission = await queueMission(mission.id, organisationId);
  } catch (error) {
    const { error: missionFailureError } = await db
      .from("missions")
      .update({
    ...
    if (missionFailureError) {
      console.error("Unable to mark unqueued direct assignment as failed", missionFailureError);
    }
  ...
  return {
    ...
    handoff: handoffData as EmployeeHandoff,
...
export async function createAiCeoCommandMission(
  input: CreateAiCeoCommandMissionInput,

  organisationId = COSSA_ORGANISATION_ID,
): Promise<DirectEmployeeMissionResult> {
  const objective = requireNonEmptyValue(input.objective, "CEO command");

  return createDirectEmployeeMission(
    {
      employeeKey: "ai-ceo",

      title: `CEO command: ${objective.slice(0, 100)}`,
      ...
      context: {
        command_source: "owner_ceo_command",

        requested_coordination: true,
...
export interface ControlledWorkforceRunInput {
  ...
  employee: Pick<AiEmployee, "id" | "employee_key" | "name" | "title" | "status">;

  provider: WorkforceExecutionProvider;

  modelName: string;

  executionKind?: WorkforceExecutionKind;
...
function compactPriorOutputs(outputs: string[]): string[] {
  return outputs
    ...
    .filter(Boolean)
    .slice(-WORKFORCE_MAX_PRIOR_OUTPUTS)
    .map((output) => compactText(output, WORKFORCE_MAX_PRIOR_OUTPUT_CHARS));
}

function compactAuthorisedEvidenceForRun(evidence: string[]): string[] {
  return evidence
    .map((item) => item.trim())
    .filter(Boolean)
    .slice(0, WORKFORCE_MAX_EVIDENCE_ITEMS)
    .map((item) => compactText(item, WORKFORCE_MAX_EVIDENCE_CHARS));
}

function inferExecutionKind(provider: WorkforceExecutionProvider): WorkforceExecutionKind {
  if (provider === "cossa_tool") {
    return "tool";
  }

  if (provider === "internal_rule") {
...
export async function mergeHandoffRetainedRecordIds(
  ...
  if (readError) {
    throw createDatabaseError("Unable to load workforce handoff identifiers", readError);
  }

  if (!existing) {
    throw new Error("The workforce handoff was not found while preserving record identifiers.");
  }

  const existingHandoff = existing as EmployeeHandoff;
  ...
  if (error) {
    throw createDatabaseError("Unable to preserve workforce record identifiers", error);
  }

  if (!data) {
    throw new Error("The workforce record identifiers could not be saved.");
  }

  return data as EmployeeHandoff;
}
...
async function claimPendingHandoff({
  ...
  organisationId: string;
}): Promise<Pick<EmployeeHandoff, "id" | "mission_id" | "to_employee_id" | "status"> | null> {
  const { data, error } = await db
    .from("employee_handoffs")
    .update({
      status: "accepted",
    ...
    .eq("status", "pending")
    .select("id,mission_id,to_employee_id,status")
    .maybeSingle();

  if (error) {
    throw createDatabaseError("Unable to claim the workforce handoff", error);
  }
...
async function releaseAcceptedHandoff({
  ...
  query = expectedRunId ? query.eq("run_id", expectedRunId) : query.is("run_id", null);

  const { error } = await query;

  if (error) {
    throw createDatabaseError("Unable to release the workforce handoff", error);
  }
}
...
export async function startControlledWorkforceRun(
  ...
  if (input.employee.status !== "active") {
    throw new Error(`${input.employee.name} is not active and cannot start a workforce run.`);
  }
  ...
  const canonicalWorkerKey = canonicalEmployeeKey(input.employee.employee_key);

  if (canonicalWorkerKey === "lead-hunter" && input.provider !== "cossa_tool") {
    throw new Error(
      "Lead Hunter must execute through the authorised Cossa Lead Hunter tool, not through a generic language-model provider.",
    );
  }

  const executionKind = input.executionKind ?? inferExecutionKind(input.provider);

  const startedAt = new Date().toISOString();

  const claimedHandoff = await claimPendingHandoff({
    handoffId: input.handoff.id,
  ...
  let createdRun: MissionRun | null = null;
  ...
  try {
    const {
      ...
      .insert({
        ...
        input: {
          ...
          handoff_context: compactContextRecord(input.handoff.context),

          prior_reviewable_outputs: compactPriorOutputs(input.priorOutputs),

          authorised_evidence: compactAuthorisedEvidenceForRun(input.authorisedEvidence ?? []),

          context_limits: {
            prior_outputs: WORKFORCE_MAX_PRIOR_OUTPUTS,

            prior_output_chars: WORKFORCE_MAX_PRIOR_OUTPUT_CHARS,

            evidence_items: WORKFORCE_MAX_EVIDENCE_ITEMS,

            evidence_chars: WORKFORCE_MAX_EVIDENCE_CHARS,
          },

          external_actions_enabled: false,

          ...(canonicalWorkerKey === "lead-hunter"
            ? {
                lead_hunter_execution: {
                  required_engine: "authenticated_server_tool",

                  route: "/api/lead-hunter/search",

                  generic_llm_search_allowed: false,

                  fabricated_prospects_allowed: false,
    ...
    if (!run) {
      throw new Error("Unable to create the workforce run: Supabase returned no run record");
    }

    createdRun = run as MissionRun;

    const {
      data: attachedHandoff,
      ...
      .eq("organisation_id", organisationId)
      .eq("to_employee_id", input.employee.id)
      .eq("status", "accepted")
      .is("run_id", null)
    ...
    if (!attachedHandoff) {
      throw new Error("The workforce handoff changed before the run could be attached.");
    }
    ...
    if (!runningMission) {
      throw new Error("The mission is no longer in an executable queued or running state.");
    }
    ...
    if (createdRun) {
      const { error: runFailureError } = await db
        .from("mission_runs")
        .update({
          status: "failed",

          error_code: "workforce_startup_failed",
      ...
      if (runFailureError) {
        console.error("Unable to mark startup run as failed", runFailureError);
      }
    }
    ...
    try {
      await releaseAcceptedHandoff({
        ...
        expectedRunId: createdRun?.id ?? null,
      });
    } catch (releaseError) {
      console.error(
...
export async function completeControlledWorkforceRun(
  input: {
    run: Pick<
      MissionRun,
      "id" | "mission_id" | "model_provider" | "model_name" | "model_request_id"
    >;
  ...
  const content = requireNonEmptyValue(input.content, "Workforce output");

  if (input.handoff.mission_id !== input.run.mission_id) {
    throw new Error("The handoff does not belong to the workforce run mission.");
  }
  ...
  const isVisualCreativeRequest =
    canonicalWorkerKey === "creative-media-producer" &&
  ...
  if (isVisualCreativeRequest) {
    ...
    const assetType = /\bbrochure\b/i.test(requestText)
      ? "brochure"
      : /\bflyer\b/i.test(requestText)
        ? "flyer"
        : /\bbanner\b/i.test(requestText)
          ? "banner"
          : /\b(product creative|product visual)\b/i.test(requestText)
            ? "product_visual"
            : /\b(social graphic|social post|poster|advertisement)\b/i.test(requestText)
              ? "social_graphic"
              : /\bwebsite|landing[- ]page\b/i.test(requestText)
                ? "website_asset"
                : "image";
    const { data, error } = await db
  ...
  const output: ControlledReviewableOutput = {
    ...
    execution_provider: input.execution?.provider ?? input.run.model_provider,

    execution_name: input.execution?.modelName ?? input.run.model_name,
  ...
  const {
    ...
    .update({
      ...
      model_provider: input.execution?.provider ?? input.run.model_provider,

      model_name: input.execution?.modelName ?? input.run.model_name,

      model_request_id: input.execution?.requestId ?? input.run.model_request_id,
  ...
  if (runError) {
    throw createDatabaseError("Unable to save the workforce output", runError);
  }

  if (!run) {
    throw new Error("Unable to save workforce output: No matching running run was updated");
  }

  const {
    data: completedHandoff,
  ...
  const incompleteHandoffs = await listIncompleteMissionHandoffs(
    input.run.mission_id,
    organisationId,
  );

  const pendingApprovalCount = await countPendingMissionApprovals(
    input.run.mission_id,
    organisationId,
  );

  const finalStage = incompleteHandoffs.length === 0 && pendingApprovalCount === 0;

  const nextMissionStatus: MissionStatus =
    pendingApprovalCount > 0 ? "awaiting_approval" : finalStage ? "completed" : "running";
  ...
  if (missionError) {
    throw createDatabaseError("Unable to update the mission status", missionError);
  }

  return {
    run: run as MissionRun,
...
export async function failControlledWorkforceRun(
  ...
  const errorMessage =
    input.errorMessage.trim().slice(0, 1_000) ||
   ...
   */
  const nextMissionStatus: MissionStatus =
    pendingApprovalCount > 0 ? "awaiting_approval" : "queued";
  ...
  if (missionError) {
    throw createDatabaseError("Unable to keep the mission available for retry", missionError);
  }
}
...
export async function requestHighRiskApproval(
  input: HighRiskApprovalInput,

  organisationId = COSSA_ORGANISATION_ID,
): Promise<Approval> {
  const actionType = requireNonEmptyValue(input.actionType, "Action type");

  const justification = requireNonEmptyValue(input.justification, "Approval justification");
  ...
  existingQuery = input.missionId
    ? existingQuery.eq("mission_id", input.missionId)
  ...
  if (existingApprovalError) {
    throw createDatabaseError("Unable to check existing high-risk approval", existingApprovalError);
  }
  ...
  if (error) {
    throw createDatabaseError("Unable to create high-risk approval request", error);
  }

  if (!data) {
    throw new Error("Unable to create high-risk approval request");
  }
...
export async function decideApproval(
  ...
  if (approval.action_type === "review_growth_coordination_output" && approval.mission_id) {
    const incompleteHandoffs = await listIncompleteMissionHandoffs(
      approval.mission_id,
      organisationId,
    );
    ...
    const nextStatus: MissionStatus =
      decision === "approved" && incompleteHandoffs.length === 0 ? "completed" : "running";

    const { error: legacyMissionError } = await db
      .from("missions")
      .update({
        status: nextStatus,
  ...
  if (approval.mission_id) {
    ...
    if (pendingApprovalCount === 0) {
      ...
      const nextStatus: MissionStatus = incompleteHandoffs.length > 0 ? "running" : "completed";
...
export async function installCossaGrowthWorkforce(
  ...
  const existing = await listEmployees(organisationId);

  await migrateUnambiguousLegacyEmployeeKeys(existing, organisationId);

  const afterLegacyMigration = await listEmployees(organisationId);

  await synchroniseKnownProfiles(afterLegacyMigration, organisationId);

  const refreshedExisting = await listEmployees(organisationId);

  const canonicalExistingKeys = new Set(
    refreshedExisting.map((employee) => canonicalEmployeeKey(employee.employee_key)),
  );

  const missingProfiles = COSSA_GROWTH_WORKFORCE.filter(
    (profile) => !canonicalExistingKeys.has(profile.employee_key),
  );
  ...
  if (missingProfiles.length > 0) {
    ...
    if (error) {
      throw createDatabaseError("Unable to install the Cossa AI workforce", error);
    }
  }

  return listEmployees(organisationId);
}
...
async function createCollaborationMission(
  definition: WorkforceMissionDefinition,

  input: CreateCoordinationMissionInput,

  organisationId = COSSA_ORGANISATION_ID,
): Promise<CoordinationMissionResult> {
  const objective = requireNonEmptyValue(input.objective, "Mission objective");

  const employees = await installCossaGrowthWorkforce(organisationId);

  const employeeByKey = employeeMapByCanonicalKey(employees);

  const missingKeys = definition.stages
    .map((stage) => canonicalEmployeeKey(stage.employeeKey))
    .filter((key) => !employeeByKey.has(key));

  if (missingKeys.length > 0) {
    throw new Error(`The workforce is missing required employees: ${missingKeys.join(", ")}.`);
  }

  const stageEmployees = definition.stages.map(
    (stage) => employeeByKey.get(canonicalEmployeeKey(stage.employeeKey)) as AiEmployee,
  );

  const inactiveEmployees = stageEmployees.filter((employee) => employee.status !== "active");
  ...
  const assignedEmployee = stageEmployees[0];

  const mission = await createMission(
    {
      title: `${definition.prefix} ${objective.slice(0, 100)}`,
      ...
      output_schema: {
        ...
        collaboration_mode: "hand_to_hand",

        stage_order: definition.stages.map((stage, index) => ({
          stage: index + 1,

          employee_key: canonicalEmployeeKey(stage.employeeKey),
        })),
  ...
  const handoffRows = definition.stages.map((stage, index) => ({
    organisation_id: organisationId,
    ...
    from_employee_id: index === 0 ? null : stageEmployees[index - 1].id,

    to_employee_id: stageEmployees[index].id,
    ...
    context: {
      ...
      previous_employee_key:
        index === 0 ? null : canonicalEmployeeKey(definition.stages[index - 1].employeeKey),

      next_employee_key:
        index < definition.stages.length - 1
          ? canonicalEmployeeKey(definition.stages[index + 1].employeeKey)
          : null,

      workflow: definition.prefix,
  ...
  const createdHandoffs = (data ?? []) as EmployeeHandoff[];

  if (createdHandoffs.length !== definition.stages.length) {
    const { error: missionFailureError } = await db
      .from("missions")
      .update({
        status: "failed",
    ...
    if (missionFailureError) {
      console.error("Unable to mark incomplete workflow mission as failed", missionFailureError);
    }
  ...
  return {
    mission: queuedMission as Mission,
...
export async function createGrowthCoordinationMission(
  ...
  return createCollaborationMission(GROWTH_WORKFLOW_DEFINITION, input, organisationId);
}
...
export async function createStoreOperationsMission(
  ...
  return createCollaborationMission(STORE_WORKFLOW_DEFINITION, input, organisationId);
}
...
export async function createTechDeliveryMission(
  ...
  return createCollaborationMission(TECH_WORKFLOW_DEFINITION, input, organisationId);
}
...
export async function createRevenueIntelligenceMission(
  ...
  return createCollaborationMission(REVENUE_WORKFLOW_DEFINITION, input, organisationId);
}
...
export async function createCustomerReactivationMission(
  ...
  return createCollaborationMission(REACTIVATION_WORKFLOW_DEFINITION, input, organisationId);
}
...
export async function createBrokerDealIntelligenceMission(
  ...
  return createCollaborationMission(BROKER_DEAL_WORKFLOW_DEFINITION, input, organisationId);
}
...
export async function createProcurementIntelligenceMission(
  ...
  return createCollaborationMission(PROCUREMENT_WORKFLOW_DEFINITION, input, organisationId);
}
