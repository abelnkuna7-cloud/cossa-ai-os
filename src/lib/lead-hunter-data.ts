...
export const LEAD_HUNTER_EMPLOYEE_KEY = "lead-hunter";

export const LEAD_HUNTER_TOOL_PROVIDER = "cossa-lead-hunter";

export const LEAD_HUNTER_TOOL_MODEL = "authenticated-search-verification";

/* -------------------------------------------------------------------------- */
/* CORE TYPES                                                                 */
/* -------------------------------------------------------------------------- */

export type LeadHunterSector = "private" | "government" | "nonprofit" | "mixed";
...
export type LeadHunterDeliveryModel = "auto" | "physical" | "remote" | "hybrid";

export type LeadHunterSearchDepth = "economy" | "standard" | "deep";
...
export type OpportunitySize = "micro" | "small" | "medium" | "large" | "strategic" | "unknown";
...
export type ProspectSalesPriority = "hot" | "warm" | "cold" | "research";
...
export type ProcurementCurrentStatus = "active" | "expired" | "unknown" | "not_applicable";
...
export const LEAD_HUNTER_STRATEGIES: LeadHunterStrategy[] = [
  {
    ...
    companies: ["cossa_nexus_construction", "cossa_facility_services", "cossa_tech"],

    services: ["property_maintenance", "commercial_cleaning", "website_design"],
...

  {
    ...
    companies: ["cossa_nexus_construction", "cossa_facility_services"],
    ...
    industries: ["Property management", "Real estate", "Sectional-title management"],
...

  {
    ...
    companies: ["cossa_nexus_construction", "cossa_facility_services", "cossa_tech", "nexdocs"],
    ...
    recommended_locations: ["Gauteng", "Limpopo", "Mpumalanga", "North West"],
    ...
    objectives: ["find_customers", "find_projects", "find_active_tenders", "find_rfqs"],
...

  {
    ...
    industries: ["Religious organisations", "Nonprofit", "Community services"],
    ...
    recommended_locations: [...PRIORITY_GAUTENG_LOCATIONS, "South Africa"],
    ...
    objectives: ["find_customers", "find_projects", "find_weak_websites", "find_branding_gaps"],
...

  {
    ...
    services: ["website_design", "seo", "digital_marketing", "lead_generation", "crm"],
...

  {
    ...
    services: ["logo_design", "branding", "website_design", "digital_marketing"],
    ...
    objectives: ["find_customers", "find_branding_gaps", "find_marketing_gaps"],
...

  {
    ...
    objectives: ["find_customers", "find_marketing_gaps", "find_recurring_contracts"],
...

  {
    ...
    industries: ["Government", "Municipal services", "Public infrastructure"],
    ...
    objectives: ["find_active_tenders", "find_rfqs", "find_supplier_registrations"],
...

  {
    ...
    industries: ["Government", "Healthcare", "Education", "Public infrastructure"],
...

  {
    ...
    industries: ["Property", "Education", "Retail", "Local government", "Professional services"],
    ...
    objectives: ["find_customers", "find_projects", "find_immediate_cashflow"],
...
export const DEFAULT_LEAD_HUNTER_REQUEST: LeadHunterSearchRequest = {
  ...
  locations: ["Pretoria", "Centurion", "Midrand", "Johannesburg", "Gauteng", "South Africa"],
  ...
  cities: ["Pretoria", "Centurion", "Midrand", "Johannesburg"],
...
function cleanLongText(value: unknown, maximumLength: number): string | null {
...
function uniqueTexts(values: unknown, maximumItems = 50): string[] {
...
export function countIndependentEvidenceSources(evidence: ProspectEvidence[]): {
  count: number;
  domains: string[];
} {
  const keys = evidence.map(evidenceSourceKey).filter((value): value is string => Boolean(value));
...
function countOfficialEvidenceSources(evidence: ProspectEvidence[]): number {
...
function sameEmail(first: string | null, second: string | null): boolean {
  return Boolean(first && second && first.toLowerCase() === second.toLowerCase());
}

function similarCompanyName(first: string, second: string): boolean {
  const normalise = (value: string) =>
    value
      .toLowerCase()
      .replace(/\b(pty|ltd|limited|inc|cc|company|holdings|group|south africa)\b/g, "")
      .replace(/[^a-z0-9]/g, "");
...
function createClientId(): string {
  ...
  return `prospect-${Date.now()}-${Math.random().toString(36).slice(2)}`;
}
...
export function isPhysicalService(service: LeadHunterServiceCategory): boolean {
  return PHYSICAL_SERVICE_CATEGORIES.includes(service);
}

export function isRemoteService(service: LeadHunterServiceCategory): boolean {
  return REMOTE_SERVICE_CATEGORIES.includes(service);
}

export function inferDeliveryModel(services: LeadHunterServiceCategory[]): LeadHunterDeliveryModel {
...
export function maxQueriesForDepth(depth: LeadHunterSearchDepth): number {
  return (
    SEARCH_DEPTH_OPTIONS.find((option) => option.value === depth)?.maximumQueries ??
    DEFAULT_MAX_SEARCH_QUERIES
  );
}

export function minimumDepthForServiceCount(serviceCount: number): LeadHunterSearchDepth {
...
function trimServicesToHardMaximum(
  ...
  return [...new Set(services)].slice(0, maxQueriesForDepth("deep"));
}
...
function extractMissionField(
  ...
  const pattern = new RegExp(`${field}\\s*:\\s*(.+?)${boundary}`, "i");

  return cleanText(instruction.match(pattern)?.[1]);
}

function extractMissionBooleanField(instruction: string, fieldPattern: string): boolean | null {
  const match = instruction.match(
    new RegExp(`\\b(?:${fieldPattern})\\s*:\\s*(yes|no|true|false|on|off)\\b`, "i"),
  );
...
function inferCompaniesFromInstruction(instruction: string): LeadHunterCompany[] {
  const matches: LeadHunterCompany[] = [];

  const companyField = extractMissionField(instruction, "Company");
...
function inferServicesFromInstruction(instruction: string): LeadHunterServiceCategory[] {
  const serviceField = extractMissionField(instruction, "Services");
   ...
   */
  const searchable = serviceField ?? instruction;
  ...
  const patterns: Array<{
  ...
  }> = [
    {
      ...
      pattern: /\bconstruction\b|\bbuilding contractor\b|\bbuilding work\b/i,
    },

    {
      service: "renovation",

      pattern: /\brenovation(?:s)?\b|\brefurbishment\b|\bremodelling\b/i,
    },
...

    {
      ...
      pattern: /\bpainting\b|\brepainting\b|\bpaint contractor\b/i,
    },

    {
      service: "tiling",

      pattern: /\btiling\b|\btile installation\b/i,
    },

    {
      service: "ceilings",

      pattern: /\bceilings?\b|\bceiling installation\b|\bceiling repairs?\b/i,
    },

    {
      service: "roofing",

      pattern: /\broofing\b|\broof repairs?\b|\broof replacement\b/i,
    },

    {
      service: "plumbing",

      pattern: /\bplumbing\b|\bplumber\b|\bwater leak\b/i,
    },

    {
      service: "facility_management",

      pattern: /\bfacilit(?:y|ies)\s+management\b|\bfacility services\b/i,
    },
...

    {
      ...
      pattern: /\blandscaping\b|\bgarden services?\b|\bgrounds maintenance\b/i,
    },

    {
      service: "waste_management",

      pattern: /\bwaste\s+management\b|\bwaste removal\b/i,
    },
...

    {
      ...
      pattern: /\blogo\s+(?:design|redesign|upgrade|revamp)\b|\bnew logo\b|\bbad logo\b/i,
    },

    {
      service: "branding",

      pattern: /\bbranding\b|\bbrand\s+identity\b|\bbrand redesign\b/i,
    },

    {
      service: "seo",

      pattern: /\bseo\b|\bsearch engine optimi[sz]ation\b|\bgoogle ranking\b/i,
    },

    {
      service: "digital_marketing",

      pattern: /\bdigital\s+marketing\b|\bonline marketing\b|\bmarketing campaign\b/i,
    },
...

    {
      ...
      pattern: /\bgoogle\s+business\s+profile\b|\bgoogle business\b|\bgbp\b/i,
    },

    {
      service: "lead_generation",

      pattern: /\blead\s+generation\b|\bcustomer acquisition\b|\bfind customers\b/i,
    },

    {
      service: "crm",

      pattern: /\bcrm\b|\bcustomer relationship management\b|\bsales pipeline\b/i,
    },
...

    {
      ...
      pattern: /\bbusiness\s+documents?\b|\bdocument generation\b|\bnexdocs\b/i,
    },

    {
      service: "quotations",

      pattern: /\bquotations?\b|\bquote systems?\b|\bquote documents?\b/i,
    },

    {
      service: "proposals",

      pattern: /\bbusiness proposals?\b|\bproposal documents?\b/i,
    },

    {
      service: "contracts",

      pattern: /\bcontracts?\b|\bcontract documents?\b/i,
    },

    {
      service: "ecommerce",

      pattern: /\be-?commerce\b|\bonline store\b|\becommerce website\b/i,
    },
  ];
...
function inferLocationsFromInstruction(instruction: string): string[] {
  const locationField = extractMissionField(instruction, "Location");
  ...
  return knownLocations.filter((location) =>
    new RegExp(`\\b${location.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`, "i").test(instruction),
  );
}
...
function inferBuyerTargetsFromInstruction(instruction: string): string[] {
...
function inferObjectivesFromInstruction(instruction: string): LeadHunterObjective[] {
  const objectives: LeadHunterObjective[] = [];

  const text = instruction.toLowerCase();

  if (/\b(customer|customers|client|clients|buyers?|prospects?|leads?)\b/.test(text)) {
    objectives.push("find_customers");
  }

  if (/\b(project|projects|work available|job opportunities|small works?)\b/.test(text)) {
    objectives.push("find_projects");
  }

  if (/\b(tender|tenders|bid|bids|etender)\b/.test(text)) {
    objectives.push("find_active_tenders");
  }

  if (/\b(rfq|request for quotation|quotation request)\b/.test(text)) {
    objectives.push("find_rfqs");
  }

  if (/\b(supplier registration|supplier database|vendor registration)\b/.test(text)) {
    objectives.push("find_supplier_registrations");
  }

  if (/\b(subcontract|subcontractor|subcontracting)\b/.test(text)) {
    objectives.push("find_subcontracting");
  }

  if (/\b(partner|partnership|referral partner|strategic partner)\b/.test(text)) {
    objectives.push("find_partners");
  }

  if (
    /\b(outdated website|bad website|weak website|website problem|website redesign)\b/.test(text)
  ) {
    objectives.push("find_weak_websites");
  }

  if (/\b(branding gap|branding problem|logo problem|bad logo|missing logo)\b/.test(text)) {
    objectives.push("find_branding_gaps");
  }

  if (/\b(inactive marketing|weak marketing|poor online presence|social media gap)\b/.test(text)) {
    objectives.push("find_marketing_gaps");
  }

  if (/\b(technology gap|automation gap|manual process|technology problem)\b/.test(text)) {
    objectives.push("find_technology_gaps");
  }

  if (/\b(maintenance|repairs?|property maintenance)\b/.test(text)) {
    objectives.push("find_maintenance_needs");
  }

  if (
    /\b(cleaning contract|commercial cleaning|office cleaning|industrial cleaning)\b/.test(text)
  ) {
    objectives.push("find_cleaning_contracts");
  }

  if (/\b(recurring|retainer|monthly contract|repeat revenue|long-term contract)\b/.test(text)) {
  ...
  if (/\b(search everything|everything relevant|all opportunities|any opportunity)\b/.test(text)) {
    objectives.push("search_everything_relevant");
  }
...
function inferRevenueModeFromInstruction(instruction: string): LeadHunterRevenueMode | null {
  const text = instruction.toLowerCase();
  ...
  if (/\b(easy wins?|low pursuit effort|easy to close|reachable customers?)\b/.test(text)) {
    return "easy_wins";
  }
  ...
  if (/\b(high value|large contract|large project|major deal|high-value)\b/.test(text)) {
    return "high_value";
  }
...
function inferSectorFromInstruction(instruction: string): LeadHunterSector | null {
  const text = instruction.toLowerCase();
  ...
  const nonprofit =
    /\b(nonprofit|non-profit|ngo|church|charity|community centre|community center)\b/.test(text);

  const count = [government, privateSector, nonprofit].filter(Boolean).length;
...
function inferSearchDepthFromInstruction(instruction: string): LeadHunterSearchDepth | null {
  if (
    /\b(deep search|deep research|investigate thoroughly|comprehensive search)\b/i.test(instruction)
  ) {
    return "deep";
  }

  if (/\b(standard search|balanced search|normal search)\b/i.test(instruction)) {
    return "standard";
  }
...
function applyInstructionIntent(
  ...
  const inferredCompanies = inferCompaniesFromInstruction(instruction);

  if (inferredCompanies.length > 0) {
    next.companies = inferredCompanies;
  }

  const inferredServices = inferServicesFromInstruction(instruction);

  if (inferredServices.length > 0) {
    next.services = inferredServices;
  }

  const inferredLocations = inferLocationsFromInstruction(instruction);

  if (inferredLocations.length > 0) {
    next.locations = inferredLocations;
  }

  const inferredBuyerTargets = inferBuyerTargetsFromInstruction(instruction);

  if (inferredBuyerTargets.length > 0) {
    next.organisation_types = inferredBuyerTargets;
  }

  const inferredObjectives = inferObjectivesFromInstruction(instruction);

  if (inferredObjectives.length > 0) {
    next.objectives = inferredObjectives;
  }

  const inferredRevenueMode = inferRevenueModeFromInstruction(instruction);

  if (inferredRevenueMode) {
    next.revenue_mode = inferredRevenueMode;
  }

  const inferredSector = inferSectorFromInstruction(instruction);

  if (inferredSector) {
    next.sector = inferredSector;

    if (inferredSector === "government") {
    ...
    if (inferredSector === "nonprofit") {
      next.include_private_sector = false;

      next.include_government_sector = false;

      next.include_nonprofits = true;
    }
  }

  const inferredDepth = inferSearchDepthFromInstruction(instruction);

  if (inferredDepth) {
    next.search_depth = inferredDepth;
  }

  const privateFieldValue = extractMissionBooleanField(instruction, "Private sector");

  const governmentFieldValue = extractMissionBooleanField(instruction, "Government");
  ...
  if (explicitPrivateOnly && !explicitGovernmentOnly) {
    next.sector = "private";
    ...
    next.include_nonprofits = false;
  } else if (explicitGovernmentOnly && !explicitPrivateOnly) {
  ...
  const negativeClauses =
    instruction.match(/\b(?:do not|don't|exclude|without|no)\b[^.!;\n]*/gi) ?? [];

  const negativeText = negativeClauses.join(" ").toLowerCase();

  const excludedServices = new Set<LeadHunterServiceCategory>();
  ...
  if (/\btechnology\b|\btech\b/.test(negativeText)) {
    ["website_design", "crm", "ai_automation", "ecommerce", "google_business_profile"].forEach(
      (service) => excludedServices.add(service as LeadHunterServiceCategory),
    );
  }
  ...
  if (/\bmarketing\b/.test(negativeText)) {
    [
      ...
      "lead_generation",
      "google_business_profile",
    ].forEach((service) => excludedServices.add(service as LeadHunterServiceCategory));
  }
  ...
  if (/\bnexdocs\b|\bbusiness documents?\b/.test(negativeText)) {
    ["business_documents", "quotations", "proposals", "contracts"].forEach((service) =>
      excludedServices.add(service as LeadHunterServiceCategory),
    );

    if (next.companies) {
      next.companies = next.companies.filter((company) => company !== "nexdocs");
    }
  }

  if (next.services && excludedServices.size > 0) {
    const filtered = next.services.filter((service) => !excludedServices.has(service));
  ...
  const resultsField = extractMissionField(instruction, "Results");

  const resultCount = safeNumber(resultsField?.match(/\d+/)?.[0]);

  if (resultCount !== null) {
    next.result_count = Math.max(1, Math.min(MAX_HUNT_RESULTS, Math.round(resultCount)));
  }

  const minimumScoreField = extractMissionField(instruction, "Minimum score");

  const minimumScore = safeNumber(minimumScoreField?.match(/\d+/)?.[0]);

  if (minimumScore !== null) {
    next.minimum_score = clampScore(minimumScore);
  }

  const requireOpportunitySignal = extractMissionBooleanField(
    instruction,
  ...
  if (allowsResearchProspects) {
    ...
    next.require_opportunity_signal = requireOpportunitySignal;
  }
  ...
  if (
    /\brecurring revenue\b|\bretainer\b|\bmonthly contract\b|\brepeat revenue\b/i.test(instruction)
  ) {
    next.revenue_mode = "recurring_revenue";

    next.revenue_first = true;
  }

  if (/\bhigh[-\s]value\b|\blarge contracts?\b|\bmajor opportunities\b/i.test(instruction)) {
    next.revenue_mode = "high_value";

    next.revenue_first = true;
  }

  if (/\bstrategic\b|\bframework\b|\blong[-\s]term account\b/i.test(instruction)) {
    next.revenue_mode = "strategic";
...
export function calculateProspectScore(
  prospect: Pick<
    LeadHunterProspect,
    "fit_score" | "intent_score" | "evidence_score" | "timing_score" | "contactability_score"
  >,
): number {
  const weightedScore =
...
export function calculateSalesPriority({
  ...
  if (totalScore >= 80 && contactabilityScore >= 60 && (intentScore >= 70 || timingScore >= 75)) {
    return "hot";
  }

  if (totalScore >= 65 && contactabilityScore >= 40) {
    return "warm";
  }
 ...
 */
export function calculateCommercialPriorityScore(prospect: LeadHunterProspect): number {
  const score =
    prospect.total_score * 0.25 +
    prospect.revenue_potential_score * 0.2 +
    prospect.ease_to_close_score * 0.2 +
    prospect.contactability_score * 0.1 +
    prospect.recurring_revenue_score * 0.1 +
    prospect.geographic_fit_score * 0.1 +
    prospect.intent_score * 0.05;
...
function normaliseVerificationMeta(
  ...
  const suppliedIndependentCount = safeNumber(raw?.independent_source_count);

  const suppliedDomains = uniqueTexts(raw?.corroborating_domains, 20)
    .map((value) => value.toLowerCase())
    .filter(Boolean);

  const corroboratingDomains = suppliedDomains.length > 0 ? suppliedDomains : detected.domains;

  const independentSourceCount = Math.max(
    detected.count,

    suppliedIndependentCount !== null ? Math.max(0, Math.round(suppliedIndependentCount)) : 0,
  );

  const officialSourceCount = Math.max(
    countOfficialEvidenceSources(evidence),

    Math.max(0, Math.round(safeNumber(raw?.official_source_count) ?? 0)),
  );

  return {
    independent_source_count: independentSourceCount,

    corroborating_domains: corroboratingDomains,

    official_source_count: officialSourceCount,

    source_cluster_id: cleanText(raw?.source_cluster_id) ?? cleanText(candidate.entity_cluster_id),

    cross_verified: raw?.cross_verified === true || independentSourceCount >= 2,

    verification_notes: uniqueTexts(raw?.verification_notes, 20),
  };
}
...
function normaliseProcurement(
  ...
  return {
    reference_number: cleanText(raw.reference_number),

    closing_date: safeDateString(raw.closing_date),

    briefing_date: safeDateString(raw.briefing_date),

    issuing_body: cleanText(raw.issuing_body),

    submission_method: cleanText(raw.submission_method),

    source_is_official: raw.source_is_official === true,

    service_match_verified: raw.service_match_verified === true,

    current_status: status,
  };
}

function normaliseWebsiteAudit(candidate: Partial<LeadHunterProspect>): WebsiteAuditFinding[] {
  ...
  return candidate.website_audit
    .flatMap((finding): WebsiteAuditFinding[] => {
      ...
      const type = validTypes.has(finding.type) ? finding.type : "other";

      const evidence = cleanLongText(finding.evidence, 1_000);

      const sourceUrl = normaliseWebsite(finding.source_url);
...
function normaliseAiInterpretation(
  ...
  return {
    ...
    confidence:
      raw.confidence === null || raw.confidence === undefined ? null : clampScore(raw.confidence),

    grounded_source_urls: uniqueTexts(raw.grounded_source_urls, 20)
...
function isProcurementClassification(classification: ProspectClassification | undefined): boolean {
  return classification === "tender" || classification === "supplier_opportunity";
}
...
function buildWhyContact(
  ...
  const reasons = uniqueTexts(candidate.why_contact, 10);
  ...
  if (candidate.public_phone || candidate.public_email) {
    generated.push("Verified public contact route is available.");
  }
  ...
  if (
    ...
    generated.push("A public website or conversion weakness was identified.");
  }

  if (
    signals.some((signal) => signal.type === "branding_problem" || signal.type === "missing_logo")
  ) {
    generated.push("A public branding weakness was identified.");
  }
  ...
  if (
    ...
    generated.push("The organisation shows a relevant physical-service signal.");
  }
  ...
  if (
    ...
    generated.push("A public procurement signal was identified.");
  }

  if (
    signals.some(
      (signal) =>
        signal.type === "new_branch" ||
        signal.type === "business_expansion" ||
        signal.type === "new_development",
    )
  ) {
    generated.push(
...
export function validateProspect(candidate: Partial<LeadHunterProspect>): LeadHunterProspect {
  const rejectionReasons: string[] = [];

  const organisationName = cleanText(candidate.organisation_name);

  if (!organisationName) {
    rejectionReasons.push("Organisation name is missing.");
  }

  const primarySourceUrl = normaliseWebsite(candidate.primary_source_url);

  if (!primarySourceUrl) {
    rejectionReasons.push("No valid primary public source URL was supplied.");
  }

  const evidence = Array.isArray(candidate.evidence)
    ? candidate.evidence
        .filter((item): item is ProspectEvidence =>
          Boolean(item && cleanText(item.title) && isValidPublicUrl(item.url)),
        )
        .map((item) => ({
          ...item,
          ...
          published_at: safeDateString(item.published_at),

          checked_at: safeDateString(item.checked_at) ?? new Date().toISOString(),

          excerpt: cleanLongText(item.excerpt, 2_000),

          supports: uniqueTexts(item.supports, 20),

          independent_source_key: cleanText(item.independent_source_key),

          is_official_source: item.is_official_source === true,
        }))
    : [];

  if (evidence.length === 0) {
    rejectionReasons.push("No valid public evidence source was supplied.");
  }
  ...
  if (!website && !phone && !email) {
    rejectionReasons.push("No website, public phone number or public email was verified.");
  }

  const signals = Array.isArray(candidate.signals)
    ? candidate.signals
        .filter((signal): signal is ProspectSignal =>
          Boolean(
            signal &&
        ...
        .map((signal) => ({
          ...
          title: cleanText(signal.title) ?? "Opportunity signal",

          explanation:
            cleanLongText(signal.explanation, 2_000) ?? "No signal explanation supplied.",

          evidence_url: normaliseWebsite(signal.evidence_url) as string,

          detected_at: safeDateString(signal.detected_at) ?? new Date().toISOString(),
  ...
  const contactabilityScore = clampScore(candidate.contactability_score);

  const suppliedTotalScore = safeNumber(candidate.total_score);
  ...
  const revenuePotentialScore =
    candidate.revenue_potential_score !== undefined
      ? clampScore(candidate.revenue_potential_score)
      : clampScore(totalScore * 0.55 + intentScore * 0.25 + timingScore * 0.2);

  const easeToCloseScore =
    candidate.ease_to_close_score !== undefined
      ? clampScore(candidate.ease_to_close_score)
      : clampScore(contactabilityScore * 0.45 + intentScore * 0.3 + fitScore * 0.25);

  const recurringRevenueScore =
    candidate.recurring_revenue_score !== undefined
      ? clampScore(candidate.recurring_revenue_score)
      : clampScore(
          [
            "facility_management",
            "commercial_cleaning",
            ...
            "crm",
            "ai_automation",
          ].includes(candidate.recommended_service ?? "general")
            ? 75
            : 35,
        );

  const geographicFitScore =
    candidate.geographic_fit_score !== undefined ? clampScore(candidate.geographic_fit_score) : 60;

  const verificationMeta = normaliseVerificationMeta(candidate, evidence);

  const procurement = normaliseProcurement(candidate);

  const websiteAudit = normaliseWebsiteAudit(candidate);

  const aiInterpretation = normaliseAiInterpretation(candidate);

  const requestedStatus = candidate.verification_status ?? "unverified";

  const requestedClassification = candidate.classification ?? "prospect";

  let classification: ProspectClassification =
    requestedStatus === "rejected" || requestedClassification === "rejected"
      ? "rejected"
      : requestedClassification;

  const hasAnyPublicIdentity = Boolean(website || phone || email);

  const procurementVerified = procurementCanBeVerified(classification, procurement);
   ...
   */
  if (classification === "tender" && !procurementVerified) {
    rejectionReasons.push("Tender verification is incomplete.");
  }
  ...
  if (
    ...
    rejectionReasons.some(
      (reason) =>
        reason === "Organisation name is missing." ||
        reason === "No valid primary public source URL was supplied." ||
        reason === "No valid public evidence source was supplied." ||
        reason === "No website, public phone number or public email was verified.",
    )
  ) {
  ...
  const salesPriority =
    ...
    signals.length > 0 &&
    signals.every((signal) => signal.type === "general_fit")
      ? "research"
  ...
  return {
    ...
    organisation_name: organisationName ?? "Rejected prospect",
    ...
    organisation_type: cleanText(candidate.organisation_type),
    ...
    identity_keys: uniqueTexts(candidate.identity_keys, 30).filter((value) =>
      /^(phone|email|domain|organisation):.+$/i.test(value),
    ),

    contact_page_url: normaliseWebsite(candidate.contact_page_url),

    contact_name: cleanText(candidate.contact_name),

    contact_title: cleanText(candidate.contact_title),

    decision_maker_route: cleanLongText(candidate.decision_maker_route, 2_000),
    ...
    country: cleanText(candidate.country) ?? "South Africa",

    recommended_company: candidate.recommended_company ?? "cossa_nexus_holdings",

    recommended_service: candidate.recommended_service ?? "general",

    service_fit_reason:
      cleanLongText(candidate.service_fit_reason, 2_500) ??
      "Service fit has not been sufficiently explained.",

    opportunity_summary:
      cleanLongText(candidate.opportunity_summary, 2_500) ??
      "No verified opportunity summary supplied.",

    opportunity_size: candidate.opportunity_size ?? "unknown",

    estimated_value: safeNumber(candidate.estimated_value),
    ...
    why_contact: buildWhyContact(candidate, signals, websiteAudit),
    ...
    date_verified: safeDateString(candidate.date_verified) ?? new Date().toISOString(),

    next_action:
      cleanLongText(candidate.next_action, 2_000) ??
      "Verify the organisation and identify the correct public procurement or decision-maker route.",

    outreach_angle: cleanLongText(candidate.outreach_angle, 2_000),

    duplicate_status: duplicateStatus,

    duplicate_lead_id: cleanText(candidate.duplicate_lead_id),

    rejection_reasons: [...new Set([...(candidate.rejection_reasons ?? []), ...rejectionReasons])],

    raw_provider_name: cleanText(candidate.raw_provider_name),

    raw_provider_result_id: cleanText(candidate.raw_provider_result_id),
    ...
    entity_cluster_id: cleanText(candidate.entity_cluster_id) ?? verificationMeta.source_cluster_id,
  };
}
...
export function validateSearchRequest(
  ...
  const request = applyInstructionIntent(requestInput);
  ...
  const resultCount = Math.min(
    ...
    Math.max(
      ...
      Math.round(Number(request.result_count ?? DEFAULT_HUNT_RESULTS)),
    ),
  );

  const rawServices = request.services?.length
    ? request.services
    : DEFAULT_LEAD_HUNTER_REQUEST.services;

  const services = trimServicesToHardMaximum(rawServices);

  const requestedDeliveryModel =
    request.delivery_model ?? DEFAULT_LEAD_HUNTER_REQUEST.delivery_model ?? "auto";

  const deliveryModel =
    requestedDeliveryModel === "auto" ? inferDeliveryModel(services) : requestedDeliveryModel;

  const explicitDepth = request.search_depth;

  const searchDepth = explicitDepth ?? minimumDepthForServiceCount(services.length);

  const depthQueryLimit = maxQueriesForDepth(searchDepth);

  const requestedQueryLimit = Math.max(
    1,

    Math.min(
      MAX_ALLOWED_SEARCH_QUERIES,

      Math.round(Number(request.max_search_queries ?? depthQueryLimit)),
    ),
  );

  const maximumQueries = Math.min(depthQueryLimit, requestedQueryLimit);

  const objectives =
    Array.isArray(request.objectives) && request.objectives.length > 0
      ? [...new Set(request.objectives)]
      : (DEFAULT_LEAD_HUNTER_REQUEST.objectives ?? ["find_customers"]);

  const searchEverything =
    safeBoolean(request.search_everything, false) ||
    objectives.includes("search_everything_relevant");

  const locations = uniqueTexts(request.locations, 25);

  const countries = uniqueTexts(request.countries, 15);

  const provinces = uniqueTexts(request.provinces, 12);

  const cities = uniqueTexts(request.cities, 25);

  const suburbs = uniqueTexts(request.suburbs, 30);

  const fallbackLocations = [...cities, ...provinces, ...countries];

  const tenderKeywords = uniqueTexts(request.tender_keywords, 25);

  const prospectKeywords = uniqueTexts(request.prospect_keywords, 35);
  ...
  return {
    ...
    companies: request.companies?.length
    ...
    industries: uniqueTexts(request.industries, 20),

    organisation_types: uniqueTexts(request.organisation_types, 20),

    result_count: resultCount,

    minimum_score: clampScore(request.minimum_score ?? DEFAULT_LEAD_HUNTER_REQUEST.minimum_score),
    ...
    minimum_evidence_sources: Math.max(
      ...
      Math.min(
        ...
        Math.round(Number(request.minimum_evidence_sources ?? 1)),
      ),
    ),

    tender_keywords:
      tenderKeywords.length > 0 ? tenderKeywords : DEFAULT_LEAD_HUNTER_REQUEST.tender_keywords,
    ...
    notes: cleanLongText(request.notes, 2_000),

    search_instruction:
      cleanLongText(request.search_instruction, MAX_CUSTOM_SEARCH_INSTRUCTION_LENGTH) ??
      DEFAULT_LEAD_HUNTER_REQUEST.search_instruction ??
      null,

    search_scope:
      request.search_scope ?? DEFAULT_LEAD_HUNTER_REQUEST.search_scope ?? "south_africa",
    ...
    revenue_mode:
      request.revenue_mode ?? DEFAULT_LEAD_HUNTER_REQUEST.revenue_mode ?? "quick_revenue",
    ...
    countries:
      countries.length > 0
        ? countries
        : (DEFAULT_LEAD_HUNTER_REQUEST.countries ?? ["South Africa"]),

    provinces: provinces.length > 0 ? provinces : (DEFAULT_LEAD_HUNTER_REQUEST.provinces ?? []),

    cities: cities.length > 0 ? cities : (DEFAULT_LEAD_HUNTER_REQUEST.cities ?? []),

    suburbs,

    radius_km:
      request.radius_km === null || request.radius_km === undefined
        ? null
        : Math.max(
            1,
            ...
            Math.min(
              ...
              Math.round(Number(request.radius_km)),
            ),
          ),

    search_everything: searchEverything,

    easy_wins_only: safeBoolean(
      request.easy_wins_only,
      DEFAULT_LEAD_HUNTER_REQUEST.easy_wins_only ?? true,
    ),

    revenue_first: safeBoolean(
      request.revenue_first,
      DEFAULT_LEAD_HUNTER_REQUEST.revenue_first ?? true,
    ),

    max_search_queries: maximumQueries,

    use_cached_results: safeBoolean(request.use_cached_results, true),
    ...
    cache_max_age_hours: Math.max(
      ...
      Math.min(
        ...
        Math.round(Number(request.cache_max_age_hours ?? DEFAULT_SEARCH_CACHE_HOURS)),
      ),
    ),

    exclude_competitors: safeBoolean(request.exclude_competitors, true),

    exclude_directories: safeBoolean(request.exclude_directories, true),

    exclude_expired_procurement: safeBoolean(request.exclude_expired_procurement, true),
  };
}
...
export function requestFromStrategy(
  ...
  const defaultProvinces = strategy.recommended_locations.filter((location) =>
    SOUTH_AFRICAN_PROVINCES.includes(location as (typeof SOUTH_AFRICAN_PROVINCES)[number]),
  );

  const defaultCities = strategy.recommended_locations.filter(
    (location) =>
      !SOUTH_AFRICAN_PROVINCES.includes(location as (typeof SOUTH_AFRICAN_PROVINCES)[number]) &&
  ...
  const strategyDepth =
    strategy.search_depth ?? minimumDepthForServiceCount(strategy.services.length);

  const strategyBudget = maxQueriesForDepth(strategyDepth);
  ...
  return validateSearchRequest({
    ...
    include_private_sector:
      strategy.target_sector === "private" || strategy.target_sector === "mixed",

    include_government_sector:
      strategy.target_sector === "government" || strategy.target_sector === "mixed",

    include_nonprofits:
      strategy.target_sector === "nonprofit" || strategy.target_sector === "mixed",
    ...
    search_scope: strategy.search_scope ?? "custom",

    delivery_model: strategy.delivery_model ?? "auto",

    revenue_mode: strategy.revenue_mode ?? "balanced",

    objectives: strategy.objectives ?? ["find_customers"],
    ...
    search_depth: strategyDepth,

    max_search_queries: strategyBudget,
...
export function createCustomHuntRequest({
  ...
  const resolvedDepth = searchDepth ?? minimumDepthForServiceCount(services.length);
  ...
  return validateSearchRequest({
    ...
    max_search_queries: maxQueriesForDepth(resolvedDepth),
  });
}
...
export function buildHuntSummary(requestInput: Partial<LeadHunterSearchRequest>): string[] {
  const request = validateSearchRequest(requestInput);

  const summary = [
    `Mission: ${request.search_instruction ?? "Find verified prospects"}`,

    `Sector: ${request.sector}`,

    `Companies: ${request.companies.join(", ")}`,

    `Scope: ${request.search_scope ?? "south_africa"}`,

    `Delivery model: ${request.delivery_model ?? "auto"}`,

    `Revenue mode: ${request.revenue_mode ?? "balanced"}`,

    `Search depth: ${request.search_depth ?? "economy"}`,

    `Maximum search queries: ${
      request.max_search_queries ?? maxQueriesForDepth(request.search_depth ?? "economy")
    }`,

    `Private sector: ${request.include_private_sector ? "YES" : "NO"}`,

    `Government: ${request.include_government_sector ? "YES" : "NO"}`,

    `Nonprofit: ${request.include_nonprofits ? "YES" : "NO"}`,

    `Require opportunity evidence: ${request.require_opportunity_signal ? "YES" : "NO"}`,
  ...
  if (request.objectives?.length) {
    summary.push(`Objectives: ${request.objectives.join(", ")}`);
  }

  if (request.provinces?.length) {
    summary.push(`Provinces: ${request.provinces.join(", ")}`);
  }

  if (request.cities?.length) {
    summary.push(`Cities: ${request.cities.join(", ")}`);
  }

  if (request.organisation_types.length > 0) {
    summary.push(`Buyer types: ${request.organisation_types.join(", ")}`);
  }
...
function sectorAllowedForRequest(
  ...
  if (request.sector !== "mixed" && request.sector !== sector) {
    return false;
  }
...
export async function huntProspects(
  ...
  if (sessionError) {
    throw new Error(`Lead Hunter authentication failed: ${sessionError.message}`);
  }

  if (!session) {
    throw new Error("Your session has expired. Sign in again before running the Lead Hunter.");
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(message || `Lead Hunter search failed (${response.status}).`);
  }

  const payload = (await response.json()) as Partial<LeadHunterSearchResponse>;

  const effectiveRequest = payload.request
    ? validateSearchRequest(payload.request)
    : validatedRequest;

  const prospects = Array.isArray(payload.prospects) ? payload.prospects.map(validateProspect) : [];
  ...
  const acceptedProspects = prospects
    .filter((prospect) => {
      ...
      if (!sectorAllowedForRequest(effectiveRequest, prospect.sector)) {
        return false;
      }

      if (
        !effectiveRequest.services.includes(prospect.recommended_service) ||
        !effectiveRequest.companies.includes(prospect.recommended_company)
      ) {
        return false;
      }

      if (prospect.total_score < effectiveRequest.minimum_score) {
        return false;
      }
      ...
      const independentSourceCount =
        prospect.verification_meta?.independent_source_count ??
        countIndependentEvidenceSources(prospect.evidence).count;

      if (independentSourceCount < effectiveRequest.minimum_evidence_sources) {
        return false;
      }

      if (effectiveRequest.verified_sources_only && prospect.evidence_score < 55) {
        return false;
      }
      ...
      if (effectiveRequest.require_website && !prospect.website) {
        return false;
      }

      if (
        effectiveRequest.require_opportunity_signal &&
        !prospectHasRequiredOpportunitySignal(prospect)
      ) {
        return false;
      }
      ...
      if (
        ...
        !procurementCanBeVerified(prospect.classification, prospect.procurement ?? null)
      ) {
        return false;
      }

      if (
        prospect.classification === "supplier_opportunity" &&
        !procurementCanBeVerified(prospect.classification, prospect.procurement ?? null)
      ) {
        return false;
      }
    ...
    .sort((first, second) => {
      if (effectiveRequest.revenue_first) {
        return calculateCommercialPriorityScore(second) - calculateCommercialPriorityScore(first);
      }

      return second.total_score - first.total_score;
    })
    .slice(0, effectiveRequest.result_count);

  const rejectedCount = prospects.length - acceptedProspects.length;

  const payloadSourceCount = safeNumber(payload.source_count);

  const payloadRejectedCount = safeNumber(payload.rejected_count);

  const calculatedIndependentSourceCount = new Set(
    acceptedProspects.flatMap(
      (prospect) =>
        prospect.verification_meta?.corroborating_domains ??
        countIndependentEvidenceSources(prospect.evidence).domains,
    ),
  ).size;
  ...
  return {
    ...
    completed_at: safeDateString(payload.completed_at) ?? new Date().toISOString(),

    request: effectiveRequest,

    prospects: acceptedProspects,

    source_count: payloadSourceCount ?? calculatedIndependentSourceCount,

    accepted_count: acceptedProspects.length,

    rejected_count: payloadRejectedCount ?? rejectedCount,

    warnings: Array.isArray(payload.warnings) ? uniqueTexts(payload.warnings, 30) : [],

    providers_used: Array.isArray(payload.providers_used)
      ? uniqueTexts(payload.providers_used, 15)
      : [],

    provider_diagnostics: Array.isArray(payload.provider_diagnostics)
...
export async function findCrmDuplicates(
  ...
  const { data, error } = await db
    .from("leads")
    .select("id,name,full_name,company,phone,email,source,status,score,created_at")
    .order(
      "created_at",
  ...
  if (error) {
    throw new Error(`Unable to check CRM duplicates: ${error.message}`);
  }

  const prospectPhone = normalisePhone(prospect.public_phone);

  const prospectEmail = normaliseEmail(prospect.public_email);

  return (data ?? [])
    .map((row: Record<string, unknown>) => {
      const rowName =
        cleanText(row.company) ?? cleanText(row.full_name) ?? cleanText(row.name) ?? "Unnamed lead";

      const matchReasons: string[] = [];

      if (sameEmail(prospectEmail, normaliseEmail(row.email))) {
        matchReasons.push("Same email address");
      }

      if (samePhone(prospectPhone, normalisePhone(row.phone))) {
        matchReasons.push("Same phone number");
      }

      if (similarCompanyName(prospect.organisation_name, rowName)) {
        matchReasons.push("Similar organisation name");
      }

      return {
        id: String(row.id),

        name: cleanText(row.full_name) ?? cleanText(row.name) ?? rowName,
    ...
    .filter((match: CrmDuplicateMatch) => match.match_reasons.length > 0);
}
...
function formatProspectEvidence(prospect: LeadHunterProspect): string {
  const evidenceLines = prospect.evidence.map((evidence, index) =>
    [
      `${index + 1}. ${evidence.title}`,

      `URL: ${evidence.url}`,

      evidence.publisher ? `Publisher: ${evidence.publisher}` : null,

      evidence.published_at ? `Published: ${evidence.published_at}` : null,

      `Checked: ${evidence.checked_at}`,

      evidence.excerpt ? `Evidence: ${evidence.excerpt}` : null,
    ]
      .filter(Boolean)
      .join("\n"),
  );

  const verificationMeta = prospect.verification_meta;

  const procurement = prospect.procurement;

  const websiteAuditLines = (prospect.website_audit ?? [])
  ...
  return [
    ...
    `Estimated value: ${
      prospect.estimated_value !== null ? `R${prospect.estimated_value.toFixed(2)}` : "Not verified"
    }`,
    ...
    `Commercial priority: ${calculateCommercialPriorityScore(prospect)}/100`,
    ...
    verificationMeta ? `Cross-verified: ${verificationMeta.cross_verified ? "YES" : "NO"}` : null,

    verificationMeta?.corroborating_domains.length
      ? `Corroborating domains: ${verificationMeta.corroborating_domains.join(", ")}`
      : null,

    procurement ? `Procurement reference: ${procurement.reference_number ?? "Not verified"}` : null,

    procurement ? `Procurement closing date: ${procurement.closing_date ?? "Not verified"}` : null,

    procurement ? `Procurement status: ${procurement.current_status}` : null,
    ...
    ...(prospect.why_contact.length > 0
      ? prospect.why_contact.map((reason) => `- ${reason}`)
      : ["- No additional commercial reason supplied."]),

    "",

    `Decision-maker route: ${prospect.decision_maker_route ?? "Not yet verified"}`,

    `Recommended next action: ${prospect.next_action}`,

    prospect.outreach_angle ? `Outreach angle: ${prospect.outreach_angle}` : null,

    websiteAuditLines.length > 0 ? "" : null,

    websiteAuditLines.length > 0 ? "VERIFIED WEBSITE AUDIT" : null,
 ...
 */
export function prospectEligibleForAutomaticCrmSave(prospect: LeadHunterProspect): boolean {
  if (prospect.verification_status !== "verified") {
    return false;
  }

  if (prospect.classification === "rejected") {
    return false;
  }
  ...
  if (
    isProcurementClassification(prospect.classification) &&
    !procurementCanBeVerified(prospect.classification, prospect.procurement ?? null)
  ) {
    return false;
  }
...
export async function saveProspectToCrm(
  ...
  if (prospect.duplicate_status === "excluded_existing_crm_lead") {
    throw new Error("This prospect was excluded because it already exists in CRM.");
  }

  if (!prospect.primary_source_url) {
    throw new Error("A verified public source URL is required before saving a prospect.");
  }
  ...
  const duplicateMatches = await findCrmDuplicates(prospect);

  const strongestDuplicate =
    duplicateMatches.find((match) =>
      match.match_reasons.some(
        (reason) => reason === "Same email address" || reason === "Same phone number",
      ),
    ) ??
    duplicateMatches[0] ??
    null;

  if (strongestDuplicate && !options.allowPossibleDuplicate) {
    return {
  ...
  const contactName = prospect.contact_name ?? prospect.organisation_name;
  ...
  const estimatedValue = prospect.estimated_value ?? 0;
  ...
  const { data, error } = await db
    ...
    .insert({
      ...
      location: [prospect.suburb, prospect.city, prospect.province, prospect.country]
        .filter(Boolean)
        .join(", "),
  ...
  if (error) {
    throw new Error(`Unable to save the verified prospect to CRM: ${error.message}`);
  }

  if (!data?.id) {
    throw new Error("The prospect was not saved because Supabase returned no lead ID.");
  }
...
export async function saveProspectsToCrm(
  ...
  const failed: SaveProspectsBatchResult["failed"] = [];

  for (const prospect of prospects) {
    try {
      const result = await saveProspectToCrm(prospect);
      ...
      failed.push({
        ...
        error: error instanceof Error ? error.message : "Unknown CRM save error.",
      });
    }
  }
...
function buildWorkforceSearchInstruction(input: LeadHunterWorkforceExecutionInput): string {
  const parts = [
    cleanLongText(input.objective, MAX_CUSTOM_SEARCH_INSTRUCTION_LENGTH),

    cleanLongText(input.instruction, 1_000),

    input.targetMarket ? `Target market: ${cleanText(input.targetMarket)}` : null,

    input.targetLocation ? `Location: ${cleanText(input.targetLocation)}` : null,

    input.targetService ? `Services: ${cleanText(input.targetService)}` : null,

    input.resultCount
      ? `Results: ${Math.max(1, Math.min(MAX_HUNT_RESULTS, Math.round(input.resultCount)))}`
      : null,

    input.minimumScore ? `Minimum score: ${clampScore(input.minimumScore)}` : null,
  ].filter((value): value is string => Boolean(cleanText(value)));

  return parts.join("\n").slice(0, MAX_CUSTOM_SEARCH_INSTRUCTION_LENGTH);
}
...
function formatWorkforceProspect(prospect: LeadHunterProspect, index: number): string {
  ...
  return [
    ...
    `Location: ${location || "Not fully verified"}`,
...
function formatLeadHunterWorkforceOutput({
  ...
  const topProspects = hunt.prospects.slice(0, 10);

  const verifiedCount = hunt.prospects.filter(
    (prospect) => prospect.verification_status === "verified",
  ).length;

  const partiallyVerifiedCount = hunt.prospects.filter(
    (prospect) => prospect.verification_status === "partially_verified",
  ).length;

  const hotCount = hunt.prospects.filter((prospect) => prospect.sales_priority === "hot").length;

  const warmCount = hunt.prospects.filter((prospect) => prospect.sales_priority === "warm").length;
  ...
  return [
    ...
    `Providers: ${
      hunt.providers_used.length > 0 ? hunt.providers_used.join(", ") : "No provider reported"
    }`,
    ...
    ...(topProspects.length > 0
      ? topProspects.map((prospect, index) => formatWorkforceProspect(prospect, index))
    ...
    ...(hunt.warnings.length > 0
      ? hunt.warnings.map((warning) => `- ${warning}`)
...
export async function runLeadHunterWorkforceHunt(
  input: LeadHunterWorkforceExecutionInput,
): Promise<LeadHunterWorkforceExecutionResult> {
  const objective = cleanLongText(input.objective, MAX_CUSTOM_SEARCH_INSTRUCTION_LENGTH);

  if (!objective) {
    throw new Error("Lead Hunter requires a real revenue-hunting objective.");
  }

  const instruction = buildWorkforceSearchInstruction(input);

  const inferredServices = inferServicesFromInstruction(instruction);

  const inferredDepth =
    input.searchDepth ??
    inferSearchDepthFromInstruction(instruction) ??
    (inferredServices.length > 0
      ? minimumDepthForServiceCount(inferredServices.length)
      : "economy");

  const inferredRevenueMode =
    input.revenueMode ?? inferRevenueModeFromInstruction(instruction) ?? "quick_revenue";
  ...
  const request = validateSearchRequest({
    ...
    result_count: input.resultCount ?? DEFAULT_WORKFORCE_HUNT_RESULTS,

    minimum_score: input.minimumScore ?? DEFAULT_LEAD_HUNTER_REQUEST.minimum_score,

    search_depth: inferredDepth,

    max_search_queries: maxQueriesForDepth(inferredDepth),

    revenue_mode: inferredRevenueMode,
  ...
  const hunt = await huntProspects(request, input.signal);
   ...
   */
  const prospectsForCrm = hunt.prospects.filter(prospectEligibleForAutomaticCrmSave);

  const saveToCrm = input.saveVerifiedProspectsToCrm !== false;

  const crm: SaveProspectsBatchResult = saveToCrm
    ? await saveProspectsToCrm(prospectsForCrm)
    : {
        created: [],
  ...
  const prospectIds = hunt.prospects.map((prospect) => prospect.id);

  const leadIds = crm.created.map((result) => result.lead_id);

  const duplicateLeadIds = crm.duplicates.map((result) => result.lead_id);

  const failedProspectIds = crm.failed.map((item) => item.prospect.id);

  const content = formatLeadHunterWorkforceOutput({
    hunt,
    crm,
  });
  ...
  return {
    ...
    retained_record_ids: {
      ...
      failed_prospect_ids: failedProspectIds,
    },

    provider: LEAD_HUNTER_TOOL_PROVIDER,

    model: LEAD_HUNTER_TOOL_MODEL,
...
export function exportProspectsToCsv(prospects: LeadHunterProspect[]): string {
  const headers = [
    "Organisation",
    "Sector",
    "Industry",
  ...
  const rows = prospects.map((prospect) => [
    ...
    calculateCommercialPriorityScore(prospect),
