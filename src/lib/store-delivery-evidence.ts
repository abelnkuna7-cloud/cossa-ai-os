...
export function parseStoreDeliveryEvidence(text: string): DeliveryEvidence {
  ...
  const dimensionKind: NonNullable<DeliveryEvidence["dimensions"]>["kind"] = /carton/i.test(dimensionMatch?.[0] ?? "") ? "carton" : /package|packed|shipping/i.test(dimensionMatch?.[0] ?? "") ? "package" : /product/i.test(dimensionMatch?.[0] ?? "") ? "product" : "unknown";
  ...
  const weightKind: NonNullable<DeliveryEvidence["weight"]>["kind"] = /shipping/i.test(weightMatch?.[0] ?? "") ? "shipping" : /package|packed|gross/i.test(weightMatch?.[0] ?? "") ? "package" : /product|net/i.test(weightMatch?.[0] ?? "") ? "product" : "unknown";
