...
export function buildDigitalProductAssistant(context: DigitalProductDraftContext) {
  ...
  const sellingPriceState =
    (context.persistedSellingPrice ?? 0) > 0
      ? "saved"
      : (context.sellingPrice ?? 0) > 0
        ? "entered_unsaved"
        : "missing";

  return {
    proposedSubtype,
    proposedSubtypeLabel: DIGITAL_PRODUCT_SUBTYPE_LABELS[proposedSubtype],
    metadataScore: readiness.score,
    missingMetadata: readiness.missing,
    sellingPriceState,
    notices: [
      "AI classification is a proposal until supported by product evidence.",
      "Unknown factual metadata must remain unknown; do not invent it to improve readiness.",
      ...(sellingPriceState === "entered_unsaved" ? ["Selling price is entered in the current form but is not yet persisted."] : []),
    ],
  };
}
