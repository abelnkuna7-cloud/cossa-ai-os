...
function workspaceLabel(pathname: string): string {
  const paths: Array<[string, string]> = [
    ["/businesses/store", "Cossa Store"],
    ["/businesses/tech", "Cossa Tech"],
    ["/businesses/construction", "Cossa Nexus Construction"],
    ["/businesses/facility-services", "Cossa Facility Services"],
    ["/businesses/nexdocs", "NexDocs"],
    ["/marketing", "Marketing & Growth"],
    ["/sales", "Sales & Revenue"],
    ["/ai/workforce", "AI Company"],
    ["/ai/orchestrator", "Cossa Orchestrator"],
...
function navigationTarget(message: string): string | null {
  ...
  const targets: Array<[RegExp, string]> = [
    [/cossa store/i, "/businesses/store"],
    [/cossa tech/i, "/businesses/tech"],
    [/construction/i, "/businesses/construction"],
    [/facility services/i, "/businesses/facility-services"],
    [/nexdocs/i, "/businesses/nexdocs"],
    [/lead hunter/i, "/sales/lead-finder"],
    [/ai (company|workforce)/i, "/ai/workforce"],
    [/command cent(?:er|re)/i, "/command-center"],
...
function parseLeadHunterMission(message: string) {
  ...
  const word = Object.keys(numberWords).find((key) =>
    new RegExp(`\\b${key}\\b`, "i").test(message),
  );
  const resultCount = Math.min(
    20,
    Math.max(1, digit ? Number(digit[1]) : word ? numberWords[word] : 5),
  );
  ...
  const location =
    message.match(/\b(Pretoria|Centurion|Gauteng|Johannesburg|Midrand|South Africa)\b/i)?.[1] ??
    "Gauteng";
  return {
...
export function CossaVoiceAssistant({ page = false }: { page?: boolean }) {
  ...
  async function send(text?: string) {
    ...
    try {
      ...
      const prior = (await listMessages(conversationId)).map((message) => ({
        role: message.role,
  ...
  if (!page && !open) {
    return (
      <button
        ...
        aria-label="Open Cossa Voice AI"
  ...
  return (
    <section className={panelClass} aria-label="Cossa Voice AI assistant">
      <header className="flex items-center gap-3 border-b border-border/60 bg-black px-4 py-3">
        <div
          className={cn(
            "flex h-9 w-9 items-center justify-center rounded-full bg-primary/15 text-primary",
            state === "LISTENING" && "animate-pulse",
            state === "SPEAKING" && "ring-2 ring-primary/40",
          )}
        >
        ...
        <div className="min-w-0 flex-1">
          <div className="font-display text-sm font-semibold text-white">
            COSSA <span className="text-primary">AI</span>
          </div>
      ...
      {settingsOpen ? (
        <div className="grid gap-3 border-b border-border/60 bg-card/40 p-4 text-xs sm:grid-cols-2">
          ...
          <div className="text-muted-foreground">
            Input: {inputProvider.replaceAll("_", " ")} · Output:{" "}
            {outputProvider.replaceAll("_", " ")} · {settings.language}
          </div>
        </div>
      ) : null}
      ...
      {error ? (
        <div
          role="alert"
          className="mx-4 mb-2 rounded-lg border border-destructive/40 bg-destructive/10 p-2 text-xs text-destructive"
        >
          {error}
        </div>
      ) : null}
      {interimTranscript ? (
        <div className="mx-4 mb-2 rounded-lg border border-primary/30 bg-primary/5 p-2 text-xs">
      ...
      <footer className="border-t border-border/60 p-3">
        <form
          ...
          className="flex items-end gap-2 rounded-xl border border-border/60 bg-card/30 p-2"
        >
          <textarea
            value={input}
            ...
            placeholder="Ask Cossa AI…"
            aria-label="Message Cossa AI"
            className="max-h-28 min-h-9 flex-1 resize-none bg-transparent px-2 py-2 text-sm outline-none"
          />
          <Button
            type="button"
            size="icon"
