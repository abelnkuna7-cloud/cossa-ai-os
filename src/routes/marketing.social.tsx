...
import { Check, Copy, ExternalLink, MessageSquareText, Sparkles } from "lucide-react";
...
function composeSavedPost(row: GrowthSocialPost) {
  return [row.content, row.cta, row.hashtags]
    .map((value) => String(value || "").trim())
    .filter(Boolean)
    .join("\n\n");
}

function CopySavedPostButton({ row }: { row: GrowthSocialPost }) {
  const [copied, setCopied] = useState(false);
  const copy = async () => {
    try {
      await navigator.clipboard.writeText(composeSavedPost(row));
      setCopied(true);
      toast.success("Saved post copied and ready to paste");
      window.setTimeout(() => setCopied(false), 2000);
    } catch {
      toast.error("Clipboard access was blocked by the browser.");
    }
  };
  return <button type="button" onClick={()=>void copy()} className="inline-flex items-center gap-1 rounded-lg border border-border px-2 py-1 text-[11px] text-primary hover:border-primary/40">{copied ? <Check className="h-3 w-3"/> : <Copy className="h-3 w-3"/>}{copied ? "Copied" : "Copy"}</button>;
}
...
function SocialAssistant() {
  ...
  const save = useMutation({
    mutationFn: async () => {
      if (!pack) throw new Error("Generate content first.");
      const ctaWithLink = [pack.cta, pack.link].map((value)=>String(value || "").trim()).filter(Boolean).join("\n");
      return growthSocialPosts.create({ platform, title: pack.title || pack.hook, content: [pack.hook, pack.caption || pack.content].filter(Boolean).join("\n\n"), hashtags: pack.hashtags, cta: ctaWithLink, status: "draft" } as Partial<GrowthSocialPost>);
    },
    onSuccess: async () => { await qc.invalidateQueries({queryKey:["growth-social-posts"]}); toast.success("AI content saved as draft with its destination link"); },
  ...
  const copyPost = async () => {
    if (!pack) return;
    const readyToPost = [pack.hook, pack.caption || pack.content, pack.cta, pack.link, pack.hashtags]
      .map((value) => String(value || "").trim())
      .filter(Boolean)
      .join("\n\n");
    try {
      ...
      setCopied(true);
      toast.success("Full post copied — message, CTA, link and hashtags are ready to paste");
      window.setTimeout(() => setCopied(false), 2500);
    } catch {
      toast.error("Clipboard access was blocked by the browser. Please allow clipboard access and try again.");
    }
  };
  return <section className="glass-card p-5 md:p-6"><div className="flex items-start gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/15 text-primary"><Sparkles className="h-5 w-5" /></div><div><h2 className="font-display text-xl font-semibold">Social Media Assistant</h2><p className="mt-1 text-sm text-muted-foreground">Give one simple instruction. Growth writes the complete post and attaches the correct approved business link.</p></div></div><div className="mt-5 grid gap-3 md:grid-cols-[180px_1fr_auto]"><select className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={platform} onChange={(e)=>setPlatform(e.target.value)}>{PLATFORMS.map(p=><option key={p} value={p}>{p}</option>)}</select><Input value={instruction} onChange={(e)=>setInstruction(e.target.value)} placeholder="e.g. NexDocs pain-point post for small businesses, strong hook" onKeyDown={(e)=>{if(e.key==="Enter"&&!generate.isPending) generate.mutate();}}/><Button disabled={generate.isPending || !instruction.trim()} onClick={()=>generate.mutate()}><Sparkles className="mr-1.5 h-4 w-4" />{generate.isPending ? "Creating…" : "Create content"}</Button></div><p className="mt-2 text-xs text-muted-foreground">Examples: “NexDocs Facebook hook”, “Cossa Tech Instagram pain-point post”, “Construction renovation lead-generation post”, “Store TikTok product caption”.</p>{pack ? <div className="mt-5 grid gap-4 rounded-xl border border-border/60 p-4 md:grid-cols-2"><label className="text-xs font-medium">Hook<Input className="mt-1" value={pack.hook} onChange={(e)=>change("hook",e.target.value)}/></label><label className="text-xs font-medium">Title<Input className="mt-1" value={pack.title} onChange={(e)=>change("title",e.target.value)}/></label><label className="text-xs font-medium md:col-span-2">Caption / message<Textarea className="mt-1 min-h-28" value={pack.caption || pack.content} onChange={(e)=>{change("caption",e.target.value);change("content",e.target.value);}}/></label><label className="text-xs font-medium">Hashtags<Input className="mt-1" value={pack.hashtags} onChange={(e)=>change("hashtags",e.target.value)}/></label><label className="text-xs font-medium">CTA<Input className="mt-1" value={pack.cta} onChange={(e)=>change("cta",e.target.value)}/></label><label className="text-xs font-medium md:col-span-2">Approved destination link<div className="mt-1 flex gap-2"><Input value={pack.link} onChange={(e)=>change("link",e.target.value)} /><a href={pack.link || "#"} target="_blank" rel="noreferrer" className="inline-flex h-10 items-center gap-1 rounded-md border border-border px-3 text-xs text-primary"><ExternalLink className="h-3.5 w-3.5"/>Open</a></div></label><label className="text-xs font-medium">Recommended best time<Input className="mt-1" value={pack.best_time} onChange={(e)=>change("best_time",e.target.value)}/></label><label className="text-xs font-medium">Visual idea<Input className="mt-1" value={pack.visual_idea} onChange={(e)=>change("visual_idea",e.target.value)}/></label><div className="flex flex-wrap gap-2 md:col-span-2"><Button onClick={()=>void copyPost()}>{copied ? <Check className="mr-1.5 h-4 w-4" /> : <Copy className="mr-1.5 h-4 w-4" />}{copied ? "Copied" : "Copy full post"}</Button><Button variant="outline" onClick={()=>save.mutate()} disabled={save.isPending}>Save as draft</Button><Button variant="outline" onClick={()=>generate.mutate()} disabled={generate.isPending}>Regenerate</Button></div><p className="text-xs text-muted-foreground md:col-span-2">Copy full post now copies the hook, caption, CTA, approved destination link and hashtags together. Best-time and visual notes stay inside Growth.</p></div> : null}</section>;
}
