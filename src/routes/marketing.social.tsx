...
function SocialAssistant() {
  ...
  const generate = useMutation({ mutationFn: () => generateSocialContentPack({ instruction, platform }), onSuccess: ({pack}) => setPack(pack), onError: (e) => toast.error(e instanceof Error ? e.message : "Generation failed") });
  ...
  const change = (key: keyof SocialContentPack, value: string) => setPack((p) => p ? {...p,[key]:value} : p);
  return <section className="glass-card p-5 md:p-6"><div className="flex items-start gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/15 text-primary"><Sparkles className="h-5 w-5" /></div><div><h2 className="font-display text-xl font-semibold">Social Media Assistant</h2><p className="mt-1 text-sm text-muted-foreground">Give one simple instruction. The assistant should return the usable post — not make you build it field by field.</p></div></div><div className="mt-5 grid gap-3 md:grid-cols-[180px_1fr_auto]"><select className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={platform} onChange={(e)=>setPlatform(e.target.value)}>{PLATFORMS.map(p=><option key={p} value={p}>{p}</option>)}</select><Input value={instruction} onChange={(e)=>setInstruction(e.target.value)} placeholder="e.g. NexDocs pain-point post for small businesses, strong hook" onKeyDown={(e)=>{if(e.key==="Enter"&&!generate.isPending) generate.mutate();}}/><Button disabled={generate.isPending || !instruction.trim()} onClick={()=>generate.mutate()}><Sparkles className="mr-1.5 h-4 w-4" />{generate.isPending ? "Creating…" : "Create content"}</Button></div><p className="mt-2 text-xs text-muted-foreground">Examples: “NexDocs Facebook hook”, “Cossa Tech Instagram pain-point post”, “Construction renovation lead-generation post”, “Store TikTok product caption”.</p>{pack ? <div className="mt-5 grid gap-4 rounded-xl border border-border/60 p-4 md:grid-cols-2"><label className="text-xs font-medium">Hook<Input className="mt-1" value={pack.hook} onChange={(e)=>change("hook",e.target.value)}/></label><label className="text-xs font-medium">Title<Input className="mt-1" value={pack.title} onChange={(e)=>change("title",e.target.value)}/></label><label className="text-xs font-medium md:col-span-2">Caption / message<Textarea className="mt-1 min-h-28" value={pack.caption || pack.content} onChange={(e)=>{change("caption",e.target.value);change("content",e.target.value);}}/></label><label className="text-xs font-medium">Hashtags<Input className="mt-1" value={pack.hashtags} onChange={(e)=>change("hashtags",e.target.value)}/></label><label className="text-xs font-medium">CTA<Input className="mt-1" value={pack.cta} onChange={(e)=>change("cta",e.target.value)}/></label><label className="text-xs font-medium">Recommended best time<Input className="mt-1" value={pack.best_time} onChange={(e)=>change("best_time",e.target.value)}/></label><label className="text-xs font-medium">Visual idea<Input className="mt-1" value={pack.visual_idea} onChange={(e)=>change("visual_idea",e.target.value)}/></label><div className="flex flex-wrap gap-2 md:col-span-2"><Button onClick={()=>save.mutate()} disabled={save.isPending}>Save as draft</Button><Button variant="outline" onClick={()=>generate.mutate()} disabled={generate.isPending}>Regenerate</Button></div></div> : null}</section>;
}

function SocialPostsPage() {
  return <CrudWorkspace<GrowthSocialPost>
    title="Social Media" tagline="Ask for content. Get content." description="Use the Social Media Assistant first. The records below are for saving, scheduling and measuring approved content — not for making you write the marketing yourself." icon={MessageSquareText} queryKey="growth-social-posts" fetch={growthSocialPosts.list} create={growthSocialPosts.create} update={growthSocialPosts.update} remove={growthSocialPosts.remove} singular="social post"
