...
const MAX_CACHED_SEARCH_AGE_MS =
  168 *
  60 *
  60 *
  1_000;
...
const KNOWN_SOUTH_AFRICAN_CITY_PROVINCES:
  Record<
    string,
    string
  > = {
...
function validateRequest(
  ...
  const sector =
    ...
    VALID_SECTORS.has(
      candidate.sector as
        LeadHunterSearchRequest["sector"],
    )
      ? candidate.sector
      : "mixed";
  ...
  if (
    ...
    return {
      ...
      error:
        "Enable at least one target sector: private, government or nonprofit.",
    };
  }
...
function independentSourceKeyForUrl(
  url: string,
): string | null {
  const hostname =
    getHostname(
...
function htmlHasWhatsappLink(
  html: string,
): boolean {
  return (
    /https?:\/\/(?:wa\.me|api\.whatsapp\.com)\//i.test(
      html,
    ) ||
    /whatsapp:/i.test(
      html,
...
function htmlHasViewportMeta(
  html: string,
): boolean {
  return /<meta\b[^>]*name=["']viewport["'][^>]*>/i.test(
    html,
  );
}

function htmlHasMetaDescription(
  html: string,
): boolean {
  return /<meta\b[^>]*name=["']description["'][^>]*content=["'][^"']{20,}["'][^>]*>/i.test(
    html,
  );
}

function htmlHasSchemaMarkup(
  html: string,
): boolean {
  return (
...
function validateProcurement(
  ...
  const submissionMethod =
    cleanText(
      content.match(
        PROCUREMENT_SUBMISSION_PATTERN,
      )?.[0],
    );
  ...
  const issuingBody =
    cleanText(
      content.match(
        PROCUREMENT_ISSUING_BODY_PATTERN,
      )?.[1],
    );
...
function createWebsiteAuditFindings(
  ...
  if (
    !inspection.usesHttps
  ) {
    add(
      "missing_https",
      "high",
      "The inspected public page was served without HTTPS.",
    );
  }

  if (
    !inspection.hasViewportMeta
  ) {
    add(
      "mobile_issue",
      "medium",
  ...
  if (
    ...
    add(
      "missing_whatsapp",
      "low",
      "No WhatsApp link was detected on the inspected page.",
    );
  }

  if (
    inspection.responseTimeMs !==
      null &&
    inspection.responseTimeMs >
      5_000
  ) {
  ...
  if (
    ...
    (
      title.length <
        10 ||
      /^(home|welcome|official website)$/i.test(
        title.trim(),
      )
    )
  ) {
...
function assessCandidate(
    ...
    return {
      ...
      reasons: [
        `A current procurement notice matches the selected service${
          procurement.matchedServices.length >
          1
            ? "s"
            : ""
  ...
  if (
    ...
    const auditFindings =
      createWebsiteAuditFindings(
        candidate,
        inspection,
      );

    const meaningfulAuditGap =
      auditFindings.some(
        (
          finding,
        ) =>
...
function procurementMetaFromValidation(
  ...
  return {
    ...
    briefing_date:
      validation.briefingDate
        ?.toISOString() ??
      null,

    issuing_body:
      validation.issuingBody,

    submission_method:
      validation.submissionMethod,

    source_is_official:
      validation.sourceIsOfficial,

    service_match_verified:
      validation.hasSelectedService,
...
function initialVerificationMeta(
  evidence:
    ProspectEvidence[],
): ProspectVerificationMeta {
  const domains =
...
function createDefaultAiInterpretation():
  AiInterpretationMeta {
  /*
...
function createProspect(
  ...
  const procurement =
    procurementMetaFromValidation(
      classification,
      procurementValidation,
    );

  const verificationMeta =
    initialVerificationMeta(
      evidence,
    );
          ...
          ...new Set([
            ...
            ...(
              normaliseOrganisationKey(
                ...
                ? [
                    `organisation:${normaliseOrganisationKey(
                      organisationName,
                    )}`,
                  ]
                : []
...
function prospectsLikelySameEntity(
  ...
  const firstOrganisation =
    normaliseOrganisationKey(
      first.organisation_name,
    );

  const secondOrganisation =
    normaliseOrganisationKey(
      second.organisation_name,
    );

  return Boolean(
    firstOrganisation &&
    secondOrganisation &&
    firstOrganisation.length >=
      5 &&
    secondOrganisation.length >=
      5 &&
    (
      firstOrganisation ===
        secondOrganisation ||
      firstOrganisation.includes(
        secondOrganisation,
      ) ||
      secondOrganisation.includes(
        firstOrganisation,
      )
    ),
  );
...
function mergeSignals(
  ...
  for (
    ...
    for (
      ...
      const key =
        `${signal.type}:${signal.evidence_url.toLowerCase()}`;
...
function procurementIsVerifiedForProspect(
  prospect:
    LeadHunterProspect,
): boolean {
  if (
...
function enrichEntityClusters(
  ...
  while (
    ...
    const verificationMeta =
      buildVerificationMetaForCluster(
        clusterId,
        mergedEvidence,
      );
    ...
    for (
      ...
      const procurementVerified =
        procurementIsVerifiedForProspect(
          prospect,
        );
...
async function loadExistingCrmLeadIdentities(
  ...
  const query =
    new URLSearchParams({
      select:
        "id,name,full_name,company,phone,email",
  ...
  try {
    const response =
      await fetch(
        `${environment.supabaseUrl}/rest/v1/leads?${query}`,
        {
          headers: {
            apikey:
              environment.supabaseKey,
      ...
      existing.find(
        (
          ...
          Boolean(
            ...
            lead.organisationKey.length >=
              6 &&
            (
              organisationKey ===
                lead.organisationKey ||
              organisationKey.includes(
                lead.organisationKey,
              ) ||
              lead.organisationKey.includes(
                organisationKey,
              )
            ),
          ),
      ...
      handlers: {
        POST:
          async ({
             ...
             * independent sources.
             */
            const clusteredProspects =
              enrichEntityClusters(
                createdProspects,
              );
             ...
             */
            const crmIdentities =
              await loadExistingCrmLeadIdentities(
                token,
                environment,
              );
