...
export const Route = createFileRoute("/sales/analytics")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Live sales metrics from your pipeline, leads and quotations." },
...
function AnalyticsPage() {
  ...
  const acceptedRate = quoteRows.length ? Math.round((quoteRows.filter((q) => q.status === "accepted").length / quoteRows.length) * 100) : 0;
  ...
  const stats = [
    ...
    { label: "Open deals", value: oppRows.filter((o) => !["won", "lost"].includes(o.stage)).length },
  ];
      ...
      <section className="glass-card relative overflow-hidden p-6 md:p-8">
        ...
        <div className="relative">
          <div className="flex items-center gap-2">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary/15 text-primary gold-glow"><BarChart3 className="h-5 w-5" /></div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {stats.map((s) => (
          <div key={s.label} className="glass-card p-5">
            ...
            <div className="mt-1 font-display text-3xl font-semibold text-gradient-gold">{s.value}</div>
          </div>
        ))}
      </div>
    </div>
