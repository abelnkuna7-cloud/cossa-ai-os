...
function ForecastPage() {
  ...
  for (const o of rows) {
    const key = o.expected_close
      ? new Date(o.expected_close).toISOString().slice(0, 7)
      : "unscheduled";
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-6">
      <section className="glass-card relative overflow-hidden p-6 md:p-8">
        ...
        <div className="relative">
          ...
          <p className="mt-1 text-muted-foreground">
            Rolled up from every open opportunity, weighted by probability.
          </p>
        </div>
      </section>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="glass-card p-4">
          ...
          <div className="mt-1 font-display text-2xl font-semibold">{rows.length}</div>
        </div>
        <div className="glass-card p-4">
          <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
            Open value
          </div>
          <div className="mt-1 font-display text-2xl font-semibold">{fmtCurrency(totalValue)}</div>
        </div>
        <div className="glass-card p-4">
          <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
            Weighted
      ...
      <section className="glass-card p-6">
        ...
        {isLoading ? (
          ...
          <div className="mt-4 text-muted-foreground text-sm">
            No opportunities yet. Add one to see the forecast.
          </div>
        ) : (
