...
function env() {
  ...
  const publishableKey = value(process.env.SUPABASE_PUBLISHABLE_KEY)
...
async function serviceRequest(environment: ReturnType<typeof env>, path: string, init: RequestInit = {}) {
  if (!environment.supabaseUrl || !environment.serviceRoleKey) throw new Error("creative_server_not_configured");
...
async function requireMember(request: Request, environment: ReturnType<typeof env>): Promise<{ userId: string } | Response> {
  if (!environment.supabaseUrl || !environment.publishableKey) {
    return json({ error: "Supabase authentication is not configured for Creative Media.", code: "auth_not_configured" }, 503);
  }
  const token = bearer(request);
  if (!token) return json({ error: "Sign in to use Creative Media generation.", code: "unauthorized" }, 401);
  ...
  if (!userResponse.ok) return json({ error: "Your session could not be verified.", code: "unauthorized" }, 401);
  const user = (await userResponse.json()) as { id?: string };
  if (!user.id) return json({ error: "Your session could not be verified.", code: "unauthorized" }, 401);

  if (!environment.serviceRoleKey) {
    return json({
      error: "Creative Media server storage is not configured. Add SUPABASE_SERVICE_ROLE_KEY to the protected Vercel environment.",
  ...
  if (!membership.ok) return json({ error: "Workspace membership could not be verified.", code: "membership_check_failed" }, 502);
  const rows = (await membership.json()) as Array<{ role?: string }>;
  if (!rows[0]?.role) return json({ error: "Cossa workspace membership is required.", code: "forbidden" }, 403);
...
function promptFor(request: CreativeRequest): string {
  ...
  return [
    ...
    request.copy_draft ? `Copy/content to represent accurately: ${request.copy_draft.slice(0, 2500)}` : "",
...
function safeProviderMessage(status: number): string {
  ...
  if (status === 429) return "OpenAI image generation is temporarily rate-limited or has no available quota.";
...
async function markBlocked(environment: ReturnType<typeof env>, requestId: string, code: string, message: string) {
  await serviceRequest(environment, `/rest/v1/creative_asset_requests?id=eq.${encodeURIComponent(requestId)}`, {
    method: "PATCH",
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (!requestId) return json({ error: "request_id is required.", code: "invalid_request" }, 400);
        ...
        if (!readResponse.ok) return json({ error: "Creative request could not be loaded.", code: "request_read_failed" }, 502);
        const records = (await readResponse.json()) as CreativeRequest[];
        const creative = records[0];
        if (!creative) return json({ error: "Creative request was not found.", code: "not_found" }, 404);
        if (["approved_asset", "delivery"].includes(creative.lifecycle_status)) {
          return json({ error: "Approved or delivered assets cannot be regenerated through this endpoint.", code: "immutable_asset_state" }, 409);
        }

        const startedAt = new Date().toISOString();
        const startResponse = await serviceRequest(environment, `/rest/v1/creative_asset_requests?id=eq.${encodeURIComponent(requestId)}`, {
          method: "PATCH",
          headers: { Prefer: "return=minimal" },
          body: JSON.stringify({
            ...
            metadata: { ...(creative.metadata ?? {}), generation_requested_by_user_id: member.userId, generation_started_at: startedAt, image_model: environment.imageModel },
          }),
        });
        if (!startResponse.ok) return json({ error: "Creative request could not enter visual generation.", code: "generation_start_failed" }, 502);
        ...
        try {
          ...
          if (!providerResponse.ok) {
            ...
            return json({ generated: false, state: "blocked", provider: "openai", error: message }, providerResponse.status === 429 ? 429 : 502);
          }

          const providerBody = (await providerResponse.json()) as { data?: Array<{ b64_json?: string; url?: string }>; id?: string };
          ...
          if (!bytes?.length) {
            ...
            return json({ generated: false, state: "blocked", provider: "openai", error: message }, 502);
          }

          const storagePath = `${new Date().toISOString().slice(0, 10)}/${requestId}/${crypto.randomUUID()}.png`;
          const uploadResponse = await serviceRequest(environment, `/storage/v1/object/${CREATIVE_BUCKET}/${storagePath}`, {
            method: "POST",
            headers: { "Content-Type": "image/png", "x-upsert": "false" },
            body: bytes,
          });
          if (!uploadResponse.ok) {
            const message = "Image was generated but could not be stored securely; the asset was not marked generated.";
          ...
          const updateResponse = await serviceRequest(environment, `/rest/v1/creative_asset_requests?id=eq.${encodeURIComponent(requestId)}`, {
            method: "PATCH",
            ...
            body: JSON.stringify({
              ...
              blocker_code: null,
              blocker_message: null,
              metadata: { ...(creative.metadata ?? {}), image_model: environment.imageModel, generation_completed_at: completedAt, generation_requested_by_user_id: member.userId },
            }),
          });
          if (!updateResponse.ok) {
            return json({ generated: false, state: "storage_only", error: "Asset was stored but the lifecycle record could not be updated. Manual review is required." }, 502);
          }
          return json({
            generated: true,
            state: "preview",
